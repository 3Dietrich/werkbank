/**
 * pitchosc.js – baut pitchosc/index.html auf (URL /pitchosc): der ZWEITE,
 ...
 * das Präfix dieses Einstiegs (pitchosc_*, aus <html data-app>, s. lib/appId.js), der
 * Erstbesuch-Stand kommt aus presets/pitchosc-config.json. BIS DAHIN teilte sich
...
//
// NUR im Original pitchosc sichtbar (@dpa ddw.md 20260803_122138 Punkt 3: "die zentrale
...
// String-Vergleich `APP === 'pitchosc'` HIER wäre falsch — new-entry.mjs schreibt beim
// Kopieren JEDES Vorkommen von `sourceSlug` (Default exakt der String "pitchosc") in
// DIESER Datei blind auf den neuen Slug um (`js.split(sourceSlug).join(slug)`, s. dortiger
// Kopf). Ein hier stehendes `'pitchosc'`-Literal würde also im Klon MIT umgeschrieben
...
// die will ich auch in beiden, pitchosc und overcord") ────────────────────────────
...
// (presets/pitchosc-config.json trägt für alle vier ISMs bereits eigene instrPos-Werte).
//
...
// `.isConnected` statt nur der Variable: im Klon (APP !== 'pitchosc') wurde die Karte
