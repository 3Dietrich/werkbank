/**
 * test3.js – baut test3/index.html auf (URL /test3): der ZWEITE,
 ...
 * das Präfix dieses Einstiegs (test3_*, aus <html data-app>, s. lib/appId.js), der
 * Erstbesuch-Stand kommt aus presets/test3-config.json. BIS DAHIN teilte sich
...
//
// NUR im Original test3 sichtbar (@dpa ddw.md 20260803_122138 Punkt 3: "die zentrale
...
// String-Vergleich `APP === 'test3'` HIER wäre falsch — new-entry.mjs schreibt beim
// Kopieren JEDES Vorkommen von `sourceSlug` (Default exakt der String "test3") in
// DIESER Datei blind auf den neuen Slug um (`js.split(sourceSlug).join(slug)`, s. dortiger
// Kopf). Ein hier stehendes `'test3'`-Literal würde also im Klon MIT umgeschrieben
...
// die will ich auch in beiden, test3 und overcord") ────────────────────────────
...
// hier fasst also nur test3-Daten an, nicht die von index.html.
...
// (presets/test3-config.json trägt für alle vier ISMs bereits eigene instrPos-Werte).
//
...
// `.isConnected` statt nur der Variable: im Klon (APP !== 'test3') wurde die Karte
