/**
 * smoke-script-test.js – baut smoke-script-test/index.html auf (URL /smoke-script-test): der ZWEITE,
 ...
 * das Präfix dieses Einstiegs (smoke-script-test_*, aus <html data-app>, s. lib/appId.js), der
 * Erstbesuch-Stand kommt aus presets/smoke-script-test-config.json. BIS DAHIN teilte sich
...
//
// NUR im Original smoke-script-test sichtbar (@dpa ddw.md 20260803_122138 Punkt 3: "die zentrale
...
// String-Vergleich `APP === 'smoke-script-test'` HIER wäre falsch — new-entry.mjs schreibt beim
// Kopieren JEDES Vorkommen von `sourceSlug` (Default exakt der String "smoke-script-test") in
// DIESER Datei blind auf den neuen Slug um (`js.split(sourceSlug).join(slug)`, s. dortiger
// Kopf). Ein hier stehendes `'smoke-script-test'`-Literal würde also im Klon MIT umgeschrieben
...
// die will ich auch in beiden, smoke-script-test und overcord") ────────────────────────────
...
// hier fasst also nur smoke-script-test-Daten an, nicht die von index.html.
...
// (presets/smoke-script-test-config.json trägt für alle vier ISMs bereits eigene instrPos-Werte).
//
...
// `.isConnected` statt nur der Variable: im Klon (APP !== 'smoke-script-test') wurde die Karte
