...
try:
    with sync_playwright() as p:
        ...
        before = pg.evaluate("() => ({ attack: window.__polysynth.state.get('adsrA'), hold: window.__polysynth.state.get('kbHold') })")
        ...
        check('adsrA' in snap['values'], f"Snapshot sollte adsrA (Amp-Env-Gruppe) enthalten: {list(snap['values'].keys())!r}")
        ...
        pg.evaluate("""() => {
            ...
            s.set('adsrA', (s.get('adsrA') || 0) + 0.2);
            s.set('kbHold', !s.get('kbHold'));
        }""")
        changed = pg.evaluate("() => ({ attack: window.__polysynth.state.get('adsrA'), hold: window.__polysynth.state.get('kbHold') })")
        ...
        after = pg.evaluate("() => ({ attack: window.__polysynth.state.get('adsrA'), hold: window.__polysynth.state.get('kbHold') })")
