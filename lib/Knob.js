...
import { hint, text as i18nText, stopText } from './i18n.js';
export class Knob {
    /**
     * @param {object} config
    ...
    constructor(config) {
        ...
        // Wert-Schriftgröße + Container-Padding (@dpa 20260718_203341: „BPM ist ein Knob,
        // dessen Value sehr wichtig ist … Schriftgröße und dessen Padding einstellbar. Das
        // kann in Knob/Fader überall mit rein"). 0/null = Default aus dem CSS (kein Sprung).
        this._valueFontSize = config.valueFontSize ?? 0;
        this._pad = config.pad ?? null;
        // Maus-Feinheit (@dpa 20260720, Punkt F): Wert-Einheiten, die eine volle Zieh-Strecke
        // abdeckt. 0 = wie bisher (volle Skala min→max über ~200px, bei weiten Ranges wie BPM
        // grob). >0 entkoppelt die Maus von der Range → feineres Ziehen; Tasten/Step bleiben.
        this._dragRange = config.dragRange ?? 0;
    ...
    // Snap-on-WRITE statt Snap-on-READ (@dpa 20260720, Punkt F): der Getter gibt den rohen Wert
    // zurück, gerastert wird beim Verstellen (Pfeile/Drag) auf das EFFEKTIVE Raster. Nur so
    // überlebt eine Shift-Fein-Verstellung UNTER step (früher schluckte der Read-Snap sie weg).
    get value() { return this._normToValue(this._normValue); }
    /** Auf ein (evtl. modifiziertes) Raster runden. step<=0 = stufenlos. */
    _snap(v, step) { return step > 0 ? Math.round(v / step) * step : v; }
    ...
    /** Wert setzen OHNE onChange – für Updates, die AUS dem State kommen (Engine: Tap/Nudge →
     *  state.set('bpm',…)). Der normale Setter würde onChange → state.set → onChange
     *  rückkoppeln (CODE_REVIEW #1, @dpa 20260719). */
    setValueSilent(v) {
        this._normValue = this._valueToNorm(v);
        this._updateVisual();
    }
    ...
    _build() {
        ...
        // Übersetzt (@dpa ddw.md 20260724, main Config „Deutsch/Englisch") — GILT NUR fürs
        // ausgelieferte Label; ein selbst vergebener Name (setMeta unten) geht NIE hier durch
        // (i18n.js-Regel: „selbst ernannte Labels bleiben unangetastet").
        i18nText(labelEl, this.label);
    ...
    /**
     * Einheitlicher Feinheits-Faktor für Maus UND Pfeiltasten (@dpa 20260720):
     *   ⇧ Shift = allgemein FEINER (×0.1) · ⌥ Alt / ⌘/Ctrl = GRÖBER (×10) · sonst 1.
     * (Löst das alte, umgekehrte Pfeiltasten-Schema ab; wie im e-Mode ist Shift überall „fein".)
     */
    _modFactor(e) {
        if (e.shiftKey) return 0.1;
        if (e.altKey || e.metaKey || e.ctrlKey) return 10;
        return 1;
    }
    _adjustValue(direction, e) {
        const step = (this._step || (this._max - this._min) / 100) * this._modFactor(e);
        let newVal = this._snap(this.value + direction * step, step);
    ...
    _buildFader(svg) {
        ...
        const len = Math.max(24, this._faderLen ?? 80);
    ...
    _applyView() {
        ...
        // Wert-Schriftgröße + Container-Padding (@dpa 20260718_203341). 0/null = CSS-Default.
        if (this._valueDisplay) {
            this._valueDisplay.style.fontSize = this._valueFontSize ? this._valueFontSize + 'px' : '';
            // Stabile Mindestbreite der Value-Box in „Ohne"-Gestalt (@dpa dd.md 20260801_2:
            // bei großen Schriften deckte die BG-Farbe den Text nicht ab; „es soll von allein
            // die richtige Größe finden … der BG soll sich aber festlegen, nicht die ganze
            // Zeit springen"). Aus dem WERTEBEREICH berechnet (min/max/Dez./Einheit), NICHT
            // aus dem gerade angezeigten Wert – sonst würde die Box bei jeder Ziffernzahl
            // (z.B. 9→10) neu springen. `ch` skaliert automatisch mit valueFontSize mit, hier
            // reicht also eine einmalige Berechnung pro Meta-Änderung (_updateVisual() bei
            // jedem Wert-Tick lässt diese Breite unangetastet). Bei eigenem formatValue ist
            // die Breite nicht vorhersagbar → dort bleibt es beim CSS-Default (min-width:100%).
            const chars = this._viewSize === 'none' ? this._valueBoxChars() : null;
            this._valueDisplay.style.minWidth = chars != null ? chars + 'ch' : '';
        }
        this.element.style.padding = this._pad != null ? this._pad + 'px' : '';
    }

    /** Breitester plausibler Text dieses Reglers in Zeichen (min/max/Dez./Einheit) - s.
     *  _applyView(). null = nicht vorhersagbar (eigenes formatValue). */
    _valueBoxChars() {
        if (this.formatValue) return null;
        const fmt = (v) => (Math.abs(v) >= 1000 ? (v / 1000).toFixed(1) + 'k' : v.toFixed(this._decimals));
        return Math.max(fmt(this._min).length, fmt(this._max).length) + (this._unit ? this._unit.length : 0);
    }
    ...
    _updateVisual() {
        ...
        let text;
        if (this.formatValue) {
            // Dezimalstellen mitgeben (@dpa 20260722_013727: „zeigt manchmal .00 an, obwohl in
            // Settings Dez.=0 gestellt ist") — ein eigenes formatValue kann sie respektieren,
            // statt eine eigene, vom Settings-Feld unabhängige Rundung zu erfinden.
            text = this.formatValue(displayVal, this._decimals);
    ...
    _onDragMove(e) {
        ...
        // Feinheits-Faktor wie bei den Pfeiltasten (@dpa 20260720): Shift ×0.1 (feiner, beide
        // Richtungen), Alt/Cmd ×10 (gröber). Wird LIVE pro Move gelesen → Shift/Alt mitten im
        // Ziehen wirkt sofort. Basis: 200 px = volle Skala (bzw. dragRange, s.u.).
        const mod = this._modFactor(e);
        // Maus-Feinheit (Punkt F): bei gesetztem dragRange deckt die volle Strecke nur diese
        // Wert-Spanne ab statt der ganzen Skala → feineres Ziehen bei weiten Ranges (BPM).
        let deltaNorm = (d / 200) * mod;
        if (this._dragRange > 0) {
            const range = (this._max - this._min) || 1;
            deltaNorm = (d / 200) * (this._dragRange / range) * mod;
        }

        this._normValue = Math.max(0, Math.min(1, this._dragStartNorm + deltaNorm));
        // Snap-on-write auf das effektive Raster (step × Modifikator): normales Ziehen bleibt
        // auf dem step-Gitter (Integer-Knobs ganzzahlig), Shift zieht feiner darunter.
        const eff = (this._step || 0) * mod;
        if (eff > 0) {
            const snapped = Math.max(this._min, Math.min(this._max, this._snap(this._normToValue(this._normValue), eff)));
            this._normValue = this._valueToNorm(snapped);
        }
    ...
    setMeta(meta) {
        ...
        // Selbst vergebener Name (KnobMetaEditor-Rename) – NIE übersetzen, und aus der
        // Übersetzungs-Merkliste nehmen, sonst dreht ein späterer Sprachwechsel den Namen
        // auf den zuletzt bekannten Shipped-Text zurück (@dpa ddw.md 20260724).
        if (meta.label !== undefined) { this.label = meta.label; if (this._labelEl) { stopText(this._labelEl); this._labelEl.textContent = meta.label; } }
        ...
        if (meta.valueFontSize !== undefined) this._valueFontSize = meta.valueFontSize;
        if (meta.pad !== undefined) this._pad = meta.pad;
        if (meta.hideValue !== undefined) this._hideValue = !!meta.hideValue;   // Wertanzeige an/aus (@dpa 20260719)
        if (meta.dragRange !== undefined) this._dragRange = meta.dragRange;      // Maus-Feinheit (Punkt F)
