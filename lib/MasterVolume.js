...
export function createMasterVolume(state) {
    ...
    // NUR ein/aus angefragt, kein eigenes Popover/keine eigene Optik, kein Scope-Creep).
    const wsBtn = document.createElement('button');
    wsBtn.type = 'button'; wsBtn.className = 'pb-btn ctrl-btn mv-ws';
    const paintWs = () => wsBtn.classList.toggle('ctrl-on', !!state.get('waveshaperOn'));
    wsBtn.textContent = 'WS';
    hint(wsBtn, 'WaveShaper-Soft-Clip — zusätzliches, weiches Sicherheitsnetz NACH dem Limiter (tanh-Sättigung statt harter Begrenzung).');
