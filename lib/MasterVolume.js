 ...
 *
 * Der Limiter (IO + Attack + Release) ist DSP-spezifisch und kein Knob-Feld: ein eigener
 * kleiner Toggle-Button daneben, dessen Rechtsklick-Popover Attack/Release zeigt.
 *
 * WaveShaper WOHNT im selben Popover, ohne eigenen Header-Button (@dpa ddw.md 20260802_234615
 * Punkt 1: „WS soll in Lim (settings) mit rein" — ein zuvor gebauter separater [WS]-Button
 * samt eigenem Optik-Popover ist deshalb wieder raus). Der Ein/Aus-Schalter ist jetzt eine
 * einfache Checkbox-Zeile in einer eigenen "WaveShaper"-Sektion zwischen Limiter und Optik.
 * Kein Key-/MIDI-Learn mehr dafür (s. Kopf-Kommentar bei `makeOptikPopover`-Aufruf unten):
 * die generische KeyMidi-Selbst-Panel-Mechanik braucht ein dauerhaft sichtbares Header-
 * Element zum Andocken, das WS nicht mehr hat.
 */
import { Knob } from './Knob.js';
import { KnobMetaEditor } from './KnobMetaEditor.js';
import { ensureAudio, setMasterDb, setLimiterOn, setLimiterAttack, setLimiterRelease, setLimiterLookahead, setWaveshaperOn } from './audioBus.js';
...
export const masterVolumeDefaults = {
    ...
    // Lookahead-Vorlauf (@dpa 20260805, Phase 2 von lib/audio/limiterProcessor.js) – wirkt
    // nur, sobald der wb-limiter-AudioWorklet geladen ist (fällt sonst still auf den
    // nativen Compressor zurück, s. audioBus.js). Default 0 = altes Verhalten unverändert.
...
export function createMasterVolume(state) {
    ...
    hint(limBtn, 'Limiter — begrenzt den Ausgang hart bei 0dB, verhindert Übersteuern. Rechtsklick: Attack/Release/WaveShaper/Optik.');
    ...
    makeOptikPopover(limBtn, 'limStyle', paintLim, {
        extraHtml: '<div class="mv-lim-sec">Limiter</div>'
            ...
            // Lookahead (@dpa 20260805, Phase 2): wirkt nur mit geladenem Worklet, s.
            // masterVolumeDefaults.limiterLookaheadMs. UI-Range 0-20ms als Default-Rahmen
            // (limits ≠ default, s. limiterProcessor.js) – kein DSP-Deckel dahinter.
            + '<div class="mv-lim-row"><label>Lookahead</label><input type="number" class="mv-lim-la" min="0" max="20" step="0.1" /><span>ms</span></div>'
            // WaveShaper-Sektion (@dpa ddw.md 20260802_234615 Punkt 1: „WS soll in Lim
            // (settings) mit rein") — schlichte Checkbox statt eigenem Toggle-Button/Optik:
            // WS hat keine sichtbare Header-Präsenz mehr, für die Farben/Größe Sinn ergäben.
            + '<div class="mv-lim-sec">WaveShaper</div>'
            + '<div class="mv-lim-row"><label>Ein/Aus</label><input type="checkbox" class="mv-lim-ws" /></div>',
        wireExtra: (pop) => {
            ...
            const laIn = pop.querySelector('.mv-lim-la');
            laIn.value = state.get('limiterLookaheadMs') ?? 0;
            laIn.addEventListener('input', () => { const v = parseFloat(laIn.value); if (Number.isFinite(v)) { state.set('limiterLookaheadMs', v); setLimiterLookahead(v); } });
            const wsIn = pop.querySelector('.mv-lim-ws');
            wsIn.checked = !!state.get('waveshaperOn');
            wsIn.addEventListener('change', () => {
                const on = wsIn.checked;
                state.set('waveshaperOn', on); setWaveshaperOn(on);
            });
        },
    });
    ...
    setLimiterLookahead(state.get('limiterLookaheadMs') ?? 0);
    setWaveshaperOn(!!state.get('waveshaperOn'));

    // limBtn mit herausgereicht (@dpa ddw.md 20260802 Punkt 7): Key-/MIDI-Learn braucht
    // das jeweilige Ensemble-KeyMidi (existiert erst NACH createMasterVolume(), s. werkbank.js/
    // werkbank-leer.js) — die Registrierung passiert darum dort, nicht hier. WS bekommt KEIN
    // eigenes Handle mehr zurück (kein Header-Button mehr, s. Datei-Kopf).
    return { element: wrap, knob, limBtn };
}
