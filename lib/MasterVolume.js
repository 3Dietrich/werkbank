...
export function createMasterVolume(state) {
    ...
    // Limiter: eigener Toggle-Button (IO), Attack/Release in seinem Rechtsklick-Popover
    // (@dpa: „Limiter mit IO Schalter, Attack ... und release ...").
    const limBtn = document.createElement('button');
    limBtn.type = 'button'; limBtn.className = 'pb-btn ctrl-btn mv-lim';
    const paintLim = () => limBtn.classList.toggle('ctrl-on', !!state.get('limiterOn'));
    limBtn.textContent = 'Lim';
    hint(limBtn, 'Limiter — begrenzt den Ausgang hart bei 0dB, verhindert Übersteuern. Rechtsklick: Attack/Release.');
    ...
    limBtn.addEventListener('contextmenu', (e) => {
        ...
        [aIn, rIn].forEach((i) => i.addEventListener('keydown', (e) => e.stopPropagation()));
