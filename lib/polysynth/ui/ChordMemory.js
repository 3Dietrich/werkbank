 ...
 *    (kein Gate, sofort bei Mousedown).
...
export class ChordMemory {
    ...
    _build() {
        ...
        for (let i = 0; i < cols * rows; i++) {
            ...
            btn.title = filled
                ? (slot.map((s) => midiToName(s.note)).join(' ') + ' — halten: Akkord klingt · loslassen: Release · Doppelklick: Kürzel · [R]: löschen')
    ...
    /** Maus runter auf einen Slot: [R] löscht belegten Slot sofort (kein Gate). Ist DIESER Slot
     *  gerade per Hold gehalten (`_heldSlot`), schaltet ein erneutes Runterdrücken ihn AUS
    ...
    _onSlotDown(i) {
        ...
        if (filled) {
            if (i === this._heldSlot) {
                this.keyboard.toggleOffChordGate();   // per Hold gehalten + nochmal gedrückt → aus
                this._heldSlot = -1;
                return;
            }
            ...
            this._downSlot = i;                // für den Loslass-Fang (_onSlotUp)
     ...
     *  Akkord wegen kbHold an, merkt sich `_heldSlot`, wofür ein erneutes Runterdrücken dann
     *  das Ausschalten ist (_onSlotDown). */
    _onSlotUp() {
