 ...
 *    gespielt wird.
 *  • [R] ist seit @dpa 20260722_194404 ein generischer `wechselButton` (w:akReset,
 *    GroupHost.js) mit 3 Stufen, deren Index direkt im State steht (this.state.get('akReset')):
 *    0 'use' = normales Speicher-Verhalten (Gate/Merken) wie oben beschrieben · 1 'over' =
 *    JEDER Klick überschreibt den geklickten Slot (egal ob leer/belegt) sofort mit dem
 *    aktuellen Akkord, kein Gate · 2 'kill' = jeder Klick löscht einen belegten Slot sofort
 *    (kein Gate). Das alte Umbenennen-per-Prompt (vormals Modus 'rename') ist damit raus.
 *
...
export class ChordMemory {
    ...
    constructor(state, keyboard, keyMidi) {
        ...
        this._gateOffTimer = null;  // pendender Auto-Release-Timer aus _remoteTrigger (s. dort)
        this._slotEls = [];
        ...
        this._build();
        state.subscribe((k) => { if (k === 'akMemory' || k === 'akMemLabels' || k === 'akReset' || k === '*') this._build(); });
    }
    ...
    /** Aktueller [R]-Modus: 0 'use' · 1 'over' · 2 'kill' — lebt direkt im w:akReset-State
     *  (GroupHost.js makeWechsel), kein eigenes Feld mehr nötig. */
    _curMode() { return Math.max(0, Math.min(2, this.state.get('akReset') | 0)); }
    ...
    _build() {
        ...
        for (let i = 0; i < cols * rows; i++) {
            ...
            hint(btn, filled
                ? (slot.map((s) => midiToName(s.note)).join(' ') + ' — halten: Akkord klingt · loslassen: Release · [R] over: überschreibt · [R] kill: löscht')
                : 'Leer — Klick merkt den aktuell aktiven Akkord · [R] over/kill wirken auch hier');
            ...
            // KEIN dblclick (@dpa 20260722, ddw.md: „das wiederholte drücken ist musikalisch
            // sinnvoll und passiert ständig ungewollt") — Umbenennen (per Prompt) ist mit dem
            // [R]-Umbau auf 'use'/'over'/'kill' (@dpa 20260722_194404) ganz entfallen.
    ...
    /** Kern-Entscheidung eines Slot-Triggers (Maus-Runter ODER Fernauslöser). [R]-Modus
     *  (_curMode()): 2 'kill' löscht einen belegten Slot sofort (kein Gate); 1 'over'
     *  überschreibt den geklickten Slot IMMER mit dem aktuellen Akkord (egal ob leer/belegt),
     *  ebenfalls kein Gate; 0 'use' ist das normale Speicher-Verhalten — ist DIESER Slot
     *  gerade per Hold gehalten (`_heldSlot`), schaltet ein erneuter Trigger ihn AUS
     *  (Umschalten wie gehabt), sonst: belegt → Gate AN, leer → aktuellen Akkord merken.
     *  Rückgabe: true, wenn ein Gate angeschlagen wurde (dann erwartet der Aufrufer später
     *  ein passendes Loslassen), sonst false. */
    _activateSlot(i) {
        const mode = this._curMode();
        ...
        if (mode === 2) {   // kill
            ...
            // @dpa ddw.md 20260724_153349, "nur bei diesem speziell": [R] over/kill sind
            // Einmal-Aktionen — nach dem Klick auf einen Speicher-Slot automatisch zurück auf
            // 'use', damit man nicht aus Versehen den nächsten Slot auch noch löscht/überschreibt.
            this.state.set('akReset', 0);
            return false;
        }
        if (mode === 1) {   // over: IMMER überschreiben, egal ob der Slot schon belegt war
            const snap = this.keyboard.snapshotChord();
            if (!snap.length) return false;
            mem[i] = snap;
            this.state.set('akMemory', mem);
            this.state.set('akReset', 0);   // s. Kommentar bei mode === 2
            return false;
        }
        if (filled) {
            if (i === this._heldSlot) {
     ...
     *
     *  BUGFIX (@dpa dd.md 20260802, "GateLen von Seq": schnelle Sequenz — 1/4 Beat — triggert
     *  denselben/einen anderen Slot schneller als 120ms hintereinander): jeder Aufruf plante
     *  BISHER einen EIGENEN, unabhängigen setTimeout — bei Retrigger-Abstand < 120ms stapelten
     *  sich mehrere Timer, und der ÄLTESTE feuerte releaseChordGate() mitten in den gerade neu
     *  angeschlagenen Akkord hinein (schnitt ihn viel zu früh ab), während spätere Timer danach
     *  ins Leere liefen. Der pendende Timer wird jetzt vor jedem neuen Trigger verworfen — reine
     *  Race-Condition-Korrektur, KEINE Änderung der 120ms-Halte-Dauer selbst. */
    _remoteTrigger(i) {
        if (this._gateOffTimer) { clearTimeout(this._gateOffTimer); this._gateOffTimer = null; }
        if (!this._activateSlot(i)) return;
        this._gateOffTimer = setTimeout(() => {
            this._gateOffTimer = null;
            const stillHeld = this.keyboard.releaseChordGate();
            this._heldSlot = stillHeld ? i : -1;
        }, 120);
    }

    /** Öffentliche Naht für den Sq-Speicher-Port (Punkt 3b, ddw.md 20260724) — derselbe
     *  Fernauslöser wie MIDI-Learn, nur von außen (Registry-write) statt von keyMidi gerufen. */
    triggerSlot(i) { this._remoteTrigger(i); }
