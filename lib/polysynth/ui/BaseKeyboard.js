...
export class BaseKeyboard {
    ...
    /** „Besonderer" MIDI-Learn (@dpa 20260722_203201: „genauso wie in Keyboard/Keyboard" —
     *  eine im normalen MIDI-Overlay gelernte Note legt die Oktave fest, die bei „Bereich" AN
     *  allein zählt, statt über einen eigenen „MIDI lernen"-Button im Panel). */
    calibrateMidiOctave(note) { this.state.set('baseMidiOctave', Math.floor(note / 12)); }
     ...
     *  (Note mod 12). Bei `baseMidiRange` AN zählen nur Noten aus genau der per MIDI-Learn
     *  kalibrierten Oktave (s. calibrateMidiOctave). */
    _onMidiMessage(msg) {
        ...
        // per MIDI-Learn kalibrierte Kanal (baseMidiChannel, werkbank.js aus der Bindung gespiegelt).
        const wantCh = this.state.get('baseMidiChannel') | 0;
