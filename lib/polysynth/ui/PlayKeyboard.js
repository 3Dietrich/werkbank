...
export class PlayKeyboard {
    ...
    constructor(state, engine) {
        ...
        this._seqNote = null;        // Sq-fernausgelöste Note (Punkt 3b), unabhängig von _pending/_active
     ...
     *  weiter, jeder Trigger löst nur die VORIGE fernausgelöste Note ab (retriggert), kein
     *  Auto-noteOff bei Stop (@dpa ddw.md „keine NoteOff bei Stop"). Bewusst UNABHÄNGIG von
     *  _active/_pending/_sounding (Hand-/Maus-Spiel) — zwei getrennte „wer hält was"-Kreise,
     *  wie triggerFromEnv/_trigHeldNote in engine.js es für den AmpEnv-Trig-Port schon macht.
     *  Velocity fix (der Step-Wert trägt hier die TONHÖHE, nicht die Lautstärke). */
    playRemote(note) {
        if (this._seqNote !== null && this._seqNote !== note) {
            this.engine.noteOff(this._seqNote);
            this._visual(this._seqNote, false);
        }
