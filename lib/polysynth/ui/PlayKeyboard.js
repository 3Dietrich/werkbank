 ...
 *
 * Akkord-Bündel + Speicher (@dpa 20260722, ddw.md Z.466–468):
 ...
 *  • AkIO (`akio`-Toggle): schaltet ALLE aktiven Tasten als EIN gebündeltes Gate. Off →
 *    ganzer Akkord verstummt (NoteOff), `_active` bleibt aber gemerkt; On → der gemerkte
 *    Akkord klingt wieder. `_onAkioChange` reagiert auf den Toggle (auch MIDI-lernbar).
 *  • Speicher: `akMemory` ist ein Slot-Raster (akMemCols×akMemRows). Klick auf einen LEEREN
 *    Slot schnappt den aktuellen Akkord hinein (Slot färbt sich); Klick auf einen BELEGTEN
 *    Slot ruft ihn ab (`_recall`: kurz NoteOff der alten Töne → NoteOn der neuen, schaltet
 *    AkIO mit an). Ist der [R]-Modus (`akReset`) aktiv, LÖSCHT ein Klick auf einen belegten
 *    Slot ihn stattdessen. `akMemory` reist als Teil des polySynthState in der Snapshot-/
 *    Config-Kette mit — keine eigene Persistenz.
 */
...
export class PlayKeyboard {
    ...
    constructor(state, engine) {
        ...
        this._slotEls = [];          // Speicher-Slot-Buttons (aktuell gerendertes Raster)
        this._akio = false;          // AkIO-Zustand (gebündeltes Gate an/aus) — b:akio-Button-getrieben
        this._resetMode = false;     // [R]-Modus (belegten Slot löschen statt abrufen) — b:akReset-getrieben
        ...
        hint(this.element, 'Spiel-Tastatur — Klick = Ton (feste Anschlagstärke). „Hold" hält Töne nach dem Loslassen, bis er wieder ausgeschaltet wird (erneuter Klick auf eine gehaltene Taste schaltet sie aus). MIDI-Eingang spielt direkt mit, Hold-bewusst. „AkIO" schaltet alle aktiven Tasten als EIN gebündeltes Gate; das Speicher-Raster merkt ganze Akkorde (leerer Slot = merken, belegter = abrufen, [R] = löschen).');
        this._rows = document.createElement('div');
        this._rows.className = 'kb-octaves';
        this.element.appendChild(this._rows);

        // Akkord-Speicher-Raster (unter den Tasten, Teil desselben u:playKb-Controls).
        this._mem = document.createElement('div');
        this._mem.className = 'kb-mem';
        this.element.appendChild(this._mem);
        ...
        state.subscribe((k) => {
            ...
            if (k === 'akMemCols' || k === 'akMemRows' || k === 'akMemory' || k === '*') this._buildMemory();
        });
    }

    // ── AkIO/[R] als Button-Aktionen (@dpa 20260722_004312: „sollten Buttons sein") ──────
    // Die latchenden b:akio/b:akReset-Buttons rufen diese Methoden (über defs onAction →
    // werkbank). Ihr Zustand lebt hier, nicht im persistierten State.
    /** b:akReset gedrückt → Reset-Modus umschalten (nur intern, der Button zeigt seinen Zustand selbst). */
    ...
    // ── Akkord-Bündel (AkIO) + Speicher (@dpa 20260722, ddw.md Z.466–468) ───────────────

    /** Snapshot des aktuellen Akkords: alle aktiven Tasten mit ihrer gemerkten Velocity,
     *  aufsteigend nach Note sortiert. Leerer Akkord → leeres Array. */
    _snapshot() {
     ...
     *  Akkord übernehmen und über AkIO als Block anschlagen. */
    _recall(notes) {
    ...
    /** Speicher-Raster (akMemCols×akMemRows) neu aufbauen. Belegte Slots zeigen die Notenzahl
     *  und färben sich (Hue aus dem tiefsten Ton), leere bleiben blank. Klemmt gegen Fehlwerte. */
    _buildMemory() {
        const cols = Math.max(1, Math.min(8, this.state.get('akMemCols') | 0 || 3));
        const rows = Math.max(1, Math.min(8, this.state.get('akMemRows') | 0 || 3));
        const mem = this.state.get('akMemory') || [];
        this._mem.innerHTML = '';
        this._mem.style.setProperty('--kb-mem-cols', cols);
        this._slotEls = [];
        for (let i = 0; i < cols * rows; i++) {
            ...
            btn.type = 'button';
            btn.className = 'kb-mem-slot' + (filled ? ' filled' : '');
            btn.textContent = filled ? String(slot.length) : '';
            if (filled) {
                btn.style.setProperty('--slot-hue', (slot[0].note * 7) % 360);   // je Akkord eigene Färbung
                btn.title = slot.map((s) => midiToName(s.note)).join(' ');
            } else {
                btn.title = 'Leerer Speicher-Slot — Klick merkt den aktuell aktiven Akkord.';
            }
            btn.addEventListener('click', () => this._onSlotClick(i));
            this._mem.appendChild(btn);
            this._slotEls.push(btn);
        }
    }

    /** Klick auf einen Speicher-Slot. [R]-Modus: belegten Slot löschen. Sonst: belegten Slot
     *  abrufen (_recall), leeren Slot mit dem aktuellen Akkord füllen (nichts aktiv → nichts tun). */
    _onSlotClick(i) {
        const mem = (this.state.get('akMemory') || []).slice();
        const slot = mem[i];
        const filled = Array.isArray(slot) && slot.length > 0;
        if (this._resetMode) {
            if (filled) { mem[i] = null; this.state.set('akMemory', mem); }   // löschen
            return;
        }
        if (filled) { this._recall(slot); return; }                          // abrufen
        const snap = this._snapshot();                                       // merken
