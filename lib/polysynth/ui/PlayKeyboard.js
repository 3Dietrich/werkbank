 ...
 */
import { midiToName, NOTE_NAMES } from '../pitch/Scaler.js';
...
export class PlayKeyboard {
    ...
    constructor(state, engine) {
        ...
        this._seqNotes = new Map();   // srcId -> Sq-fernausgelöste Note (Punkt 3b), unabhängig von _pending/_active.
                                       // Pro Quelle (Bugfix „lautes Getöse" bei 2+ aktiven Sq, ddw.md
                                       // 20260724_192304): mit EINER geteilten Variable riss ein zweiter
                                       // Sq-Klon auf demselben Ziel dem ersten ständig die Note wieder ab.
    ...
    /** „Besonderer" MIDI-Learn (@dpa 20260722_155726: „es soll nur eine Note eingegeben
     *  werden müssen, statt jede Taste einzeln … im MIDI-learn mode, nicht als extra Button"):
     *  im normalen Tasten-/MIDI-Overlay auf das Keyboard-Control gelernt, EINE gespielte Note
     *  legt `kbMidiOffset` so fest, dass GENAU diese physische Taste auf die unterste
     *  angezeigte Taste (kbStart) fällt — kein Einzel-Learn pro Taste nötig. */
    calibrateMidiOffset(note) {
        const off = Math.max(-4, Math.min(4, Math.round((this._baseMidi() - note) / 12)));
        this.state.set('kbMidiOffset', off);
    }
    ...
    _build() {
        ...
        for (let o = 0; o < octaves; o++) {
            ...
            for (let pc = 0; pc < 12; pc++) {
                ...
                // Oktave nur bei C zeigen (@dpa 20260722_172315: "spart uns zusätzlich 1/3 Platz")
                // — alle anderen Tasten nur die Tonklasse, ohne Oktavzahl.
                const id = document.createElement('div'); id.className = 'kb-id'; id.textContent = pc === 0 ? midiToName(note) : NOTE_NAMES[pc];
    ...
    /** Sq-Fernauslöser (Punkt 3b, ddw.md 20260724): legato — die Note klingt zwischen Steps
     *  weiter, jeder Trigger löst nur die VORIGE fernausgelöste Note DERSELBEN QUELLE ab
     *  (retriggert), kein Auto-noteOff bei Stop (@dpa ddw.md „keine NoteOff bei Stop").
     *  Bewusst UNABHÄNGIG von _active/_pending/_sounding (Hand-/Maus-Spiel) — zwei getrennte
     *  „wer hält was"-Kreise, wie triggerFromEnv/_trigHeldNotes in engine.js es für den
     *  AmpEnv-Trig-Port schon macht. `srcId` (Bugfix „lautes Getöse" bei 2+ aktiven Sq,
     *  ddw.md 20260724_192304): PRO Sq-Klon ein eigener Eintrag in `_seqNotes` — sonst riss
     *  ein zweiter Klon auf demselben Ziel dem ersten ständig die Note wieder ab.
     *  Velocity fix (der Step-Wert trägt hier die TONHÖHE, nicht die Lautstärke). */
    playRemote(note, srcId = '') {
        const prev = this._seqNotes.get(srcId);
        if (prev != null && prev !== note) {
            this.engine.noteOff(prev);
            this._visual(prev, false);
        }
        this.engine.noteOn(note, 100);
        this._visual(note, true);
        this._seqNotes.set(srcId, note);
    }
    ...
    _initMidi() {
        ...
        navigator.requestMIDIAccess().then((access) => {
            ...
            // UND BaseKeyboard.js' eigenen Roh-Zugriff — mit `.onmidimessage =` überschreibt
    ...
    _onMidiMessage(msg) {
        const [status, d1, d2] = msg.data, cmd = status & 0xf0, ch = (status & 0x0f) + 1;
        if (cmd !== 0x90 && cmd !== 0x80) return;   // „Controls u.a. werden ignoriert" — nur Note on/off
        // Gelernter Kanal (@dpa-Vorwurf s. PLAN_OPERA.md 1.2): 0 = alle Kanäle, sonst NUR der
        // per MIDI-Learn kalibrierte Kanal (kbMidiChannel, werkbank.js aus der Bindung gespiegelt).
        const wantCh = this.state.get('kbMidiChannel') | 0;
        if (wantCh && wantCh !== ch) return;
