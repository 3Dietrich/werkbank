...
export class AdsrCore {
     ...
     * sr: Sample-Rate
     * Rückgabe: { curve: Float32Array, duration: seconds, startOffset: seconds, holdsForever }
     *   startOffset ist negativ, wenn der Outin-Fade vor dem eigentlichen Trigger beginnt.
     *   holdsForever=true heißt: die Kurve endet NICHT bei 0, sondern hält den letzten Wert
     *   für immer (offener Gate-Sustain o.ä.) — der Aufrufer darf dafür KEIN Auto-Stop
     *   einplanen. false heißt: die Kurve ist selbst-beendend, der Aufrufer MUSS zum Ende
     *   der `duration` einen Stop/Cleanup einplanen, sonst läuft die Voice für immer weiter
     *   (@dpa dd.md 20260802, 3. Runde, Fest-Leck-Bug).
     */
    trigger(cfg, sr) {
        ...
        // Hilfsfunktion: Kurve zusammensetzen + inv anwenden. holdsForever (dd.md 20260802,
        // 3. Runde, @dpa: Fest-Bug "fächert die polyphonen Envs zeitlich auf ... hängt sich
        // irgendwann auf") — Aufrufer (engine.js spawnVoice) MUSS bei holdsForever===false
        // einen osc.stop() zum Ende der Kurve einplanen, sonst läuft die Voice nach einer
        // selbst-beendenden Hüllkurve (Fest/Trig/ADS-ohne-R o.ä.) für immer weiter (kein
        // onended, kein cleanupVoice — Voice-Leck). true nur an den beiden expliziten
        // openHold-Stellen unten UND am Standard-Ende, wenn der Pegel NICHT bei 0 landet.
        const finish = (parts, holdsForever = false) => {
            let totalLen = 0;
            ...
            return { curve: total, duration: total.length / sr, startOffset: -outinSamples / sr, holdsForever };
        };
        ...
        if (aOn && dOn && sOn && !rOn) {
            ...
            if (openHold) {
                ...
                return finish([fadeCurve, aCurveArr, dCurveArr], true);
            }
        ...
        this._lastLevel = currentLevel * inv;
        // holdsForever, wenn der Pegel NICHT bei 0 landet (sOn && !rOn — Fest ohne Release,
        // s. finish()-Kommentar oben): genau der Fall, in dem envCore.js selbst schon "für
        // immer" hält, unabhängig vom openHold-Flag oben.
        return finish(parts, currentLevel !== 0);
    }
