...
import { AdsrCore } from './envCore.js';
...
export function createPolySynthEngine(state) {
     ...
     *  dd.md 20260802: „die Env auf die ADSR übermodeln, so dass sie die selben
     *  Einstellungen hat") — dieselben Settings wie das Multi-ADSR (lib/polysynth/
     *  multiEnv.js _cfg()), NUR ohne Peak/Len/TrigMode/Nullpunkt: Peak kommt aus der
     *  Velocity (Parameter hier), Amp-Env bleibt konzeptionell immer „Gate offen bis
     *  noteOff" (trigMode fest 'gate', lenFest fest false → openHold in AdsrCore.trigger(),
     *  s. dortiger Kommentar) — kein Trig-Modus, kein Auto-Close. Defaults (defs.js) bilden
     *  das VORHERIGE Hardcoding 1:1 nach: alle Segmente an, A linear, D/R log, Skew 1,
     *  kein Invert/Verlauf — reine Erweiterung, kein Klangunterschied ohne @dpas Zutun. */
    function ampCfg(peak) {
        return {
            a: Math.max(0, state.get('ampAttack') ?? 0.01),
            d: Math.max(0, state.get('ampDecay') ?? 0.15),
            s: clamp01(state.get('ampSustain') ?? 0.7),
            r: Math.max(0, state.get('ampRelease') ?? 0.3),
            peak,
            aOn: state.get('ampAOn') !== false, dOn: state.get('ampDOn') !== false,
            sOn: state.get('ampSOn') !== false, rOn: state.get('ampROn') !== false,
            inv: !!state.get('ampInv'), verlauf: !!state.get('ampVerlauf'),
            aCurve: state.get('ampACurve') || 'lin', dCurve: state.get('ampDCurve') || 'log', rCurve: state.get('ampRCurve') || 'log',
            aSkew: state.get('ampASkew') ?? 1, dSkew: state.get('ampDSkew') ?? 1, rSkew: state.get('ampRSkew') ?? 1,
            trigMode: 'gate', lenFest: false, lenSamples: 0,
        };
    }
     ...
     *  dieselben Sonderfälle wie envCore.js 1:1 nach (Amp-Env ist immer trigMode='gate'/
     *  openHold, darum entfällt der ADR-bei-Trig-Sonderfall dort ersatzlos). */
    function scheduleAmpTriggerNative(gain, cfg, t) {
        ...
        if (!cfg.aOn && !cfg.dOn && !cfg.sOn && !cfg.rOn) {
            ...
            p.linearRampToValueAtTime(AMP_FLOOR, t + 1 / gain.context.sampleRate);
            return;
        }
        ...
        if (cfg.aOn) {
            ...
            if (!cfg.dOn && !cfg.sOn && !cfg.rOn) {
                ...
                p.linearRampToValueAtTime(AMP_FLOOR, time + 0.0005);
                return;
            }
        }
        if (cfg.dOn) {
        ...
        if (cfg.sOn) return;   // openHold: AudioParam hält den letzten geplanten Wert von selbst.
        if (cfg.rOn) {
            time += Math.max(0, cfg.r);
            rampSeg(p, cfg.rCurve, level, 0, time);
        }
        // sonst: hält implizit auf `level` (identisch zu AdsrCore ohne S/R-Segment).
    }
    ...
    function spawnVoice(note, velocity) {
        ...
        if (ampCfgIsNative(cfg)) {
            ...
            // Release oder wie hoch die Polyphonie ist.
            scheduleAmpTriggerNative(g, cfg, t);
        } else {
            const { curve, duration, startOffset } = env.trigger(cfg, audio.sampleRate);
        ...
        // bis noteOff die Release-Kurve nachreicht (releaseVoice).
        g.connect(getMaster());
        ...
        // AdsrCore-Instanz dieser Voice (releaseVoice() ruft ihre gateOff()).
        const voice = { note, oscs, damps, ratios, gain: g, peak, env, releasing: false };
     ...
     *  `release` ist bei noteOff der ampRelease-Knob (live aus dem State, s. noteOff()), bei
    ...
    function releaseVoice(voice, release) {
        ...
        try {
            ...
            const cfg = { ...ampCfg(voice.peak), r: Math.max(0.001, release) };
     ...
     *  laut klingende Noten) gewollt hart, nicht musikalisch — dafür gibt's ampRelease/
     *  Stealing bereits. */
