...
export function createPolySynthEngine(state) {
    ...
    function spawnVoice(note, velocity) {
        ...
        const { curve, duration, startOffset } = env.trigger(ampCfg(peak), audio.sampleRate);
    ...
    function releaseVoice(voice, release) {
        ...
        try {
            ...
            const { curve, duration } = voice.env.gateOff(cfg, audio.sampleRate, current);
            gain.cancelScheduledValues(t);
            gain.setValueAtTime(current, t);
            gain.setValueCurveAtTime(curve, t, duration);
