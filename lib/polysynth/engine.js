 ...
 *     zweitem, verstimmtem Oszillator pro Voice (`osc2On`/`detune`).
 *
 ...
 */
import { foldToBand, midiToFreq, harmonicSnap, NOTE_NAMES } from './pitch/Scaler.js';
...
export function createPolySynthEngine(state) {
    ...
    state.subscribe((k) => {
        ...
        // BaseFrq-Quelle verstellt → Test-Ton nachziehen UND alle gehaltenen Voices retunen
        // (deren Quantisierung hängt an der BaseFrq). harmonizeMix ändert nur das Noten-Raster,
        // nicht den Test-Ton-Sinus → dort nur retuneHeld.
        else if (k === 'baseSrc' || k === 'baseHz' || k === 'baseBand' || k === 'baseNote') { refreshTestFreq(); retuneHeld(); }
     ...
     *  ob die Voice gerade im Attack/Decay oder schon im Sustain steht — cancelAndHoldAtTime
     *  friert den Gain auf seinem AKTUELLEN (ggf. noch rampenden) Wert ein, von dort startet
     *  die Release-Rampe (kein Sprung auf den Sustain-Level nötig). Release LOG/exponentiell,
