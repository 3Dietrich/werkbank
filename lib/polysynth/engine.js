...
export function createPolySynthEngine(state) {
    ...
    // Erweitert (dd.md 20260802, 2. Runde, "alles von ADSR einfach rein!"): Modus/Fest sind
    // jetzt echte Settings statt hartcodiert. scheduleAmpTriggerNative() kennt nur den
    // openHold-Fall (Gate, kein Auto-Close) — echtes Trig ODER Fest=an (Auto-Close nach Len)
    // fällt darum auf den allgemeinen, bereits korrekten envCore.js-Pfad zurück (derselbe,
    // den Multi-ADSR für genau diese Fälle nutzt) statt hier eine zweite Implementierung
    // nachzubauen. Default bleibt trigMode='gate'/lenFest=false → unverändert nativer Pfad.
    function ampCfgIsNative(cfg) {
        return cfg.aSkew === 1 && cfg.dSkew === 1 && cfg.rSkew === 1 && !cfg.inv
            && cfg.trigMode === 'gate' && cfg.lenFest === false;
    }
