...
import { oscCoefficients, harmonicsForFreq, applyHarmonicTilt } from './audio/pulseWave.js';
...
export function createPolySynthEngine(state) {
    ...
    state.subscribe((k) => {
        ...
        // Obertonreihen-Gewichtung (oscTilt, @dpa ddw.md 20260723_230925) ist Teil der
        // GEBACKENEN Wellenform (s. bakedWave) — läuft darum über denselben Live-Update-Pfad
        // wie die Klangfarbe selbst (retimbreHeld), kein eigener Gain-Node/Mechanismus mehr.
        else if (k === 'oscEngine' || k === 'duty' || k === 'fmFeedback' || k === 'oscTilt') retimbreHeld();
    ...
    /** Gebackene PeriodicWave aus Cache holen/erzeugen (identische engine/param/tilt/N
     *  wiederverwenden) — tilt geht seit @dpa ddw.md 20260723_230925 mit in den Cache-Key
     *  ein, weil er die Koeffizienten selbst verändert (applyHarmonicTilt), nicht nur eine
     *  Laufzeit-Lautstärke. */
    function bakedWave(audio, engine, param, tilt, N) {
        const key = `${engine}_${Math.round(param * 200)}_${Math.round(tilt || 0)}_${N}`;
        let w = waveCache.get(key);
        if (!w) {
            const raw = oscCoefficients(engine, param, 0, N);
            const { real, imag } = applyHarmonicTilt(raw.real, raw.imag, tilt || 0);
    ...
    function spawnVoice(note, velocity) {
        ...
        const oscs = ratios.map((r) => {
            ...
            osc.setPeriodicWave(bakedWave(audio, engine, param, tilt, N));
        ...
        const voice = { note, oscs, ratios, gain: g, peak, releasing: false };
