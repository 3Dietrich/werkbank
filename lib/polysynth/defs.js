...
// Amp-Env (Gruppe "Amp-Env", Poly-Synth-Schritt 4, volle ADSR-Parität seit dd.md 20260802
// 2. Runde) ist das echte ADSR pro Voice — nutzt DIESELBEN Knobs/Settings/Defaults wie
// Multi-ADSR (ADSR_KNOBS/ADSR_SETTINGS_*/ADSR_DEFAULTS), nur unsuffixed (sfx='').
...
export function polySynthDefs(opts = {}) {
    ...
    //
    // VOR DEFAULTS/KNOBS deklariert (dd.md 20260802, 2. Runde, @dpa: "alles von ADSR
    // einfach rein!"): Amp-Env braucht dieselben Objekte SCHON beim Bauen von DEFAULTS/
    // KNOBS unten (per Spread, unsuffixed) — Multi-ADSR nutzt sie weiterhin unverändert
    // als Suffix-Template in multiEnv.js buildEnv() (adsrA_0, adsrA_1, …).
    const ADSR_BUTTONS = {
    ...
    const ADSR_KNOBS = {
        ...
        // curve:'log' statt 'linear' (dd.md 20260802, "Mittenstellung ist 500k?"): bei einer
        // LINEAREN Kurve über 0.01–1000000 landet ein normaler Dreh-Weg fast sofort im
        // Hunderttausender-Bereich — der praktisch relevante Bereich um 1 (0.01–1 üblich)
        // bekommt fast keine Auflösung. 'log' geht NICHT auf Kosten der Obergrenze (die
        // bleibt 1000000, keine neue Klemme!) — es verteilt nur den Dreh-Weg gleichmäßiger
        // über Größenordnungen, genau wie baseHz/baseBand oben.
    ...
    // Panel↔Settings-Umschalter (todos.md). Für Amp-Env INERT (kein flush()/Routing-Ziel
    // konsumiert es dort, s. GROUPS-Kommentar) — bewusst trotzdem mit rein, @dpa: „Nullpunkt
    // einfach 0 lassen, dann ist der als Control da, aber auf default=0.. so what?".
    ...
    const DEFAULTS = {
        ...
        // Amp-Env (Poly-Synth-Schritt 4, @dpa 20260721; volle ADSR-Parität seit dd.md
        // 20260802, 2. Runde: „alles von ADSR einfach rein!"): echtes ADSR pro Voice über
        // dieselbe envCore.js AdsrCore-Instanz wie Multi-ADSR, DIESELBEN State-Keys, nur
        // unsuffixed (sfx=''). ...ADSR_DEFAULTS liefert A/D/S/R/Peak/Kurven/Skew/Len/
        // Nullpunkt 1:1 — NUR trigMode/lenFest werden überschrieben, damit das bisherige
        // Verhalten ohne @dpas Zutun klanglich unverändert bleibt: Amp-Env war/bleibt
        // default „Gate offen bis noteOff" (ADSR_DEFAULTS selbst startet bei 'trig'/fest,
        // das passt für Multi-ADSR, nicht fürs bisherige Amp-Env-Verhalten).
        ...ADSR_DEFAULTS,
        adsrTrigMode: 'gate',
        adsrLenFest: false,
    ...
    const KNOBS = {
        ...
        // Amp-Env-Knobs: dieselben Objekte wie Multi-ADSR (ADSR_KNOBS), s. DEFAULTS-Kommentar
        // oben — A/D/S/R/Peak/Len(Ms/Beat), Labels 'A'/'D'/'S'/'R' statt 'Attack'/'Decay'/…
        // (dd.md 20260802, 2. Runde: „alles von ADSR einfach rein!").
        ...ADSR_KNOBS,
    ...
    const BUTTONS = {
        ...
        // Amp-Env „Fest" + „►" (dd.md 20260802, 3. Runde, @dpa: "Button '►' ... fehlt noch
        // (kann doch nicht sein wenn es eine echte ADSR ist..?)"): dieselben ADSR_BUTTONS-
        // Definitionen wie Multi-ADSR, unsuffixed. '►' hat bei Amp-Env KEINE einzelne Instanz
        // zum Gaten (polyphon, eine AdsrCore pro Voice, s. engine.js spawnVoice) — statt
        // Attrappe spielt der Klick einen Referenzton (Preview/Audition, phase down=noteOn/
        // up=noteOff über die echte Engine, s. onAction-Dispatcher in overcord/werkbank.js)
        // — man hört die Hüllkurve, ohne die Tastatur anzufassen.
        adsrGate: ADSR_BUTTONS.adsrGate,
        adsrLenFest: ADSR_BUTTONS.adsrLenFest,
    };
    ...
    const GROUPS = [
        ...
        {
            name: 'Amp-Env',
            // groupKind:'ADSR' (dd.md 20260802) + IDENTISCHE Keys wie eine Multi-ADSR-Instanz
            // (nur unsuffixed) → teilt sich Combo-/Snapshot-Pool UND Settings-Hook (s.
            // overcord/werkbank.js _groupKindSettings.ADSR) wirklich vollständig, nicht nur
            // dem Namen nach. Volle Parität (2. Runde, @dpa: „alles von ADSR einfach rein!"):
            // Peak/Len/Modus/LenEinheit/Kurven/Skew/Nullpunkt — siehe defs.js DEFAULTS/KNOBS-
            // Kommentare für die zwei bewusst überschriebenen Verhaltens-Defaults
            // (adsrTrigMode='gate', adsrLenFest=false, damit der Klang unverändert bleibt).
            groupKind: 'ADSR',
            knobs: ['adsrA', 'adsrD', 'adsrS', 'adsrR', 'adsrPeak', 'adsrLenMs', 'adsrLenBeat'],
            buttons: ['adsrGate', 'adsrLenFest'],
        },
        {
            name: 'Keyboard',
    ...
    return {
        ...
        // Multi-ADSR Template-Defs (ddw.md 20260725) — für createAdsrManager UND (seit
        // dd.md 20260802, 2. Runde) direkt für Amp-Env (DEFAULTS/KNOBS-Spread oben,
        // Settings-Hook in overcord/werkbank.js) — eine einzige Quelle für beide.
        ...
        ports: {
            outputs: [
                ...
                // Scope-Outputs (ddw.md 20260802_234615, „Outs (für scope): viel zu wenig, bitte
                // diverse Outputs hinzufügen (controls nicht, aber Ausgänge aller Art (envelopes,
                // OSZ, ..)") — reine Metadaten, Typ 'Value' wie multiEnv.js' Env-Output (generischer
                // Scope-Wert, kein Modulationsziel-Sonderfall). Bindung (read/hasNode/node) in
                // overcord/werkbank.js, s. dortigen Kommentar + engine.js ampEnvPeak()/
                // oscMonitorNode()-Kommentare für die Polyphonie-Design-Entscheidung.
                { id: 'ampEnv', label: 'Amp-Env', type: 'Value' },
                { id: 'osc', label: 'OSZ (roh)', type: 'Value' },
            ],
            inputs: [
