...
export function polySynthDefs(opts = {}) {
    ...
    const GROUPS = [
        ...
        },
        {
            ...
            // Escape-Hatch für den Gruppen-Snapshot (@dpa 20260724: "alles dabei ... auch der
            // Inhalt vom Speicher"): akMemory/akMemLabels hängen am Unikat-Control 'u:speicher'
            // (ChordMemory.js, per mountInGroup aus werkbank.js eingehängt) — dessen data-ctrl-
            // ID parst zu key='speicher', keinem echten State-Key. Ohne diese Liste würde der
            // Gruppen-Snapshot den gemerkten Akkord-Speicher stillschweigend NICHT sichern.
    ...
    return {
        ...
        ports: {
            ...
            inputs: [
                ...
                ...knobsToPorts(KNOBS),
                ...buttonsToPorts(BUTTONS, ['chordUp', 'chordDown', 'kbHold']),
                { id: 'speicher', label: 'Speicher-Slot', type: 'Gate', min: 0, max: 32, stepSize: 1, offAllowed: true },
                { id: 'note', label: 'Note (KB)', type: 'Value', min: 0, max: 127, stepSize: 1, offAllowed: true },
                ...
                { id: 'tonWahl', label: 'Ton-Wahl', type: 'Value', min: 1, max: 12, stepSize: 1, offAllowed: false },
            ],
        },
    };
}
