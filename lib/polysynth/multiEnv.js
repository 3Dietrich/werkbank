...
export function createEnvManager({ host, state, defs, tpl, routing, getBpm }) {
    ...
    function buildEnv(i) {
        ...
        // ── Output-PickMenu (Zielbestimmung) ──────────────────────────────────
        ...
        // PickMenu-Element in die Gruppen-Header-Zeile einfügen (neben dem Namen)
        const grpEl = host.panel.querySelector(`[data-group="${envName(i)}"]`);
        if (grpEl) {
            const header = grpEl.querySelector('.group-head');
            if (header) header.appendChild(pickMenu.element);
        }
    }
