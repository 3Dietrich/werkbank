...
export function createEnvManager({ host, state, defs, tpl, routing, getBpm }) {
    ...
    function buildEnv(i) {
        ...
        const updateKnobVisibility = () => {
            for (const [key, onKey] of [['adsrA', 'adsrAOn'], ['adsrD', 'adsrDOn'], ['adsrS', 'adsrSOn'], ['adsrR', 'adsrROn']]) {
                show(knobFor(key), st.get(onKey));
            }
            // Len-Sichtbarkeit (@dpa ddw.md 20260726, „Panel frei räumen von inaktiven
            // Elementen", korrigiert 20260727 — s. defs.js adsrLenFest-Kommentar: EIN Len-
            // Konzept für BEIDE Modi, kein separates GateLen mehr): Trig ist per Definition
            // IMMER 'fest' (kein Fest/Offen-Button nötig, Len bestimmt die feste Länge). Gate
            // zeigt den Fest/Offen-Button; Len NUR bei fest (offen braucht keine Zeitangabe —
            // läuft live bis zum echten Gate-Off, s. envCore.js). In BEIDEN Fällen zeigt Len
            // nur die zur Len-Einheit passende Variante (ms/beat).
            const isGate = st.get('adsrTrigMode') === 'gate';
            const isFest = st.get('adsrLenFest') !== false;
            const lenUnit = st.get('adsrLenUnit') || 'ms';
        ...
        // Fest/Offen-Button ist eine echte GroupHost-BUTTON (mode:'toggle') — sein isOn hängt
        // NICHT automatisch am State (anders als TOGGLES, s. defs.js kbHold-Kommentar), muss
        // also wie kbHold per setCtrlOn manuell nachgezogen werden (Reload/Config-Apply/Recall).
        const syncLenFestBtn = () => host.setCtrlOn('b:adsrLenFest' + sfx, st.get('adsrLenFest') !== false);
        updateKnobVisibility();
        syncLenFestBtn();
        // Bei Änderung der Settings-Toggles/Modus/Einheit/Fest-Button Sichtbarkeit aktualisieren
        state.subscribe((key) => {
    ...
    function flush() {
        for (let i = 0; i < engines.length; i++) {
            ...
            const active = mag > SILENT || engines[i]._gate;
            ...
            // Nullpunktversatz (@dpa ddw.md 20260727, „Bug2"): die Env kreist um IHREN eigenen
            // Nullpunkt (0) — ein Ziel mit anderem Ruhepunkt (z.B. Frequenz-Multiplikator, Ruhe
            // = 1) braucht diesen Versatz AN DER ENV, nicht als Sonderformel im Consumer (das
            // alte, jetzt zurückgebaute `1 + mod` in engine.js). NACH der Min/Max-Skalierung
            // angewandt, damit die bestehende generische Skalierung unangetastet bleibt.
            out += state.get('adsrNullpunkt' + sfx) || 0;
            routing.deliver(
