...
export class ElementSettings {
  ...
  _build() {
    ...
    panel.innerHTML = `
      ...
      <div class="kme-body">
        ...
        <!-- top Layer (@dpa ddw.md Z.875, „User-Lösung"): Stapel-Reihenfolge gegenüber
             Geschwister-Controls im selben Panel — Auslöser war der Stepseq, wo 'Steps' die
             dahinterliegenden An/Zähler/Nenner-Controls verdeckte. Cross-cutting, gilt für
             ALLE Typen (s. open()/_collect() unten, NICHT über _fieldsFor gesteuert). -->
        <div class="kme-row kme-wide" data-f="layerTop">
          <label>Oben</label>
          <input type="checkbox" class="es-layertop" title="Dieses Control vor überlappenden Geschwister-Controls anzeigen" />
        </div>
        ...
        <div class="kme-grid">
          ...
          <!-- Data-Sektion (Punkt 1, ddw.md, Sequenzer-Steps — @dpa wörtlich): Werte-Skala
               ...
               'Aus' AN (Standard) = wie bisher: Klick schaltet Steps an/aus, ein gesetzter
               Wert wird beim Ausschalten gemerkt. 'Aus' AUS = kein Sonderfall mehr, jeder
               Wert inkl. 0 ist ein echter Wert, Klick/Zug setzen ihn direkt. -->
    ...
    const live = ['.es-label', '.es-labelon', '.es-labelpos', '.es-btnmode', '.es-size', '.es-fontsize', '.es-boxsize', '.es-boxh', '.es-gap', '.es-beatsize', '.es-radius', '.es-pad', '.es-texton', '.es-textoff', '.es-textblink', '.es-memcols', '.es-memrows', '.es-slotsize', '.es-layertop', '.es-seqoff', '.es-seqmin', '.es-seqmax', '.es-seqstep'];
    live.forEach((sel) => {
  ...
  open(target) {
    ...
    show.add('layerTop');   // cross-cutting (@dpa ddw.md Z.875), gilt für ALLE Typen
    ...
    this._panel.querySelector('.es-layertop').checked = !!style.layerTop;
  ...
  _collect() {
    ...
    if (P('.es-layertop').checked) s.layerTop = true;   // cross-cutting, s. open()
