...
export class ElementSettings {
  ...
  constructor(state) {
    ...
    this._target = null;
    // Panel-Umschalter (@dpa dd.md 20260801): GroupHost setzt hier die echten Hooks ein
    // (State-Key `ctrlOffPanel`, den WIR nie selbst lesen/schreiben) — No-Ops als Default,
    // damit dieses Modul auch ohne GroupHost (z.B. in einem Selbsttest) nicht crasht.
    this.offPanelGet = () => false;
    this.onOffPanel = () => {};
    this._build();
  }
  ...
  _build() {
    ...
    panel.innerHTML = `
      <div class="kme-header">
        ...
        <!-- Panel-Umschalter (@dpa dd.md 20260801): "es braucht einen völlig neuen toggle
             für 'Panel?'" - eigenständig, NICHTS mit der Knob-Gestalt "Ohne" zu tun (die
             bleibt auf dem Panel, nur ohne Dial). Icon+Hilfstext synchronisiert open(). -->
        <button class="kme-panelbtn" type="button"></button>
      ...
      <div class="kme-body">
        ...
        <div class="kme-grid">
          <div class="kme-row" data-graphic="1" data-f="labelOn">
            <label>Label an</label>
            <input type="checkbox" class="es-labelon" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="labelPos">
          ...
          <div class="kme-row" data-graphic="1" data-f="bg">
            <label>BG</label>
          ...
          <div class="kme-row" data-graphic="1" data-f="bgOn">
            <label>BG an</label>
          ...
          <div class="kme-row" data-graphic="1" data-f="bgBlink">
          ...
          <div class="kme-row" data-graphic="1" data-f="fg">
            <label>Text</label>
          ...
          <div class="kme-row" data-graphic="1" data-f="size">
            <label>Größe</label>
            <input type="number" class="es-size" min="6" max="1000000" step="1" title="Schriftgröße im Feld (px)" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="fontSize">
            <label>Textgr.</label>
            <input type="number" class="es-fontsize" min="7" max="1000000" step="1" title="Textgröße (px)" />
          </div>
          ...
          <div class="kme-row" data-graphic="1" data-f="boxSize">
            <label>Breite</label>
            <input type="number" class="es-boxsize" min="20" max="1000000" step="2" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="boxH">
            <label>Höhe</label>
            <input type="number" class="es-boxh" min="16" max="1000000" step="2" />
          </div>
          ...
          <div class="kme-row" data-graphic="1" data-f="gap">
            <label>Abstand</label>
            <input type="number" class="es-gap" min="0" max="1000000" step="1" title="Abstand zwischen den Tasten (px)" />
          </div>
          ...
          <div class="kme-row" data-graphic="1" data-f="horiz">
          ...
          <div class="kme-row" data-graphic="1" data-f="mainColor">
          ...
          <div class="kme-row" data-graphic="1" data-f="subColor">
          ...
          <div class="kme-row" data-graphic="1" data-f="beatSize">
            <label>Beat-Größe</label>
            <input type="number" class="es-beatsize" min="4" max="1000000" step="1" title="Durchmesser eines Beat-Punkts (px)" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="radius">
            <label>Radius</label>
            <input type="number" class="es-radius" min="0" max="1" step="0.05" title="0 = eckig … 1 = rund (normierter Anteil, mathematisch gedeckelt — mehr als 'ganz rund' gibt es nicht)" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="pad">
            <label>Padding</label>
            <input type="number" class="es-pad" min="0" max="1000000" step="1" title="Innenabstand zur Hintergrundfläche (px)" />
          </div>
          ...
          <!-- Spalten/Zeilen bewusst NICHT auf die sonstige Millionen-Decke angehoben: jede
               Zelle ist ein echtes DOM-Element (Slot-Button) — anders als eine px-Zahl, die
               nur eine CSS-Eigenschaft ist, würde eine Million Spalten/Zeilen den Browser
               tatsächlich mit einer Million Buttons zum Absturz bringen. 200×200 ist immer
               noch weit jenseits jeder sinnvollen Nutzung, aber keine echte Absturzgefahr. -->
          <div class="kme-row" data-graphic="1" data-f="memCols">
            <label>Spalten</label>
            <input type="number" class="es-memcols" min="1" max="200" step="1" title="Anzahl Speicher-Spalten" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="memRows">
            <label>Zeilen</label>
            <input type="number" class="es-memrows" min="1" max="200" step="1" title="Anzahl Speicher-Zeilen" />
          </div>
          <div class="kme-row" data-f="slotSize">
            <label>Slot</label>
            <input type="number" class="es-slotsize" min="12" max="1000000" step="1" title="Kantenlänge eines Slot-Quadrats (px)" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="vg1">
          ...
          <div class="kme-row" data-graphic="1" data-f="vg2">
          ...
          <div class="kme-row" data-graphic="1" data-f="vg3">
          ...
          <!-- LevelMeter (@dpa dd.md 20260802: "6dB Unterteilungen … rms zuschaltbar … peak
               schmal … Farben für alles … clip … LimiterAnzeige … hektik") — alle mit *
               markierten Panel-Punkte als Controls. 'fg'/'bg' oben decken schon Bar/Hintergrund
               ab, hier nur das NEUE: Skala, RMS-Linie, Peak-Hold-Breite/-Haltezeit, gestufter
               6dB-Verlauf, Limiter-Reduktionsanzeige, Reaktionsgeschwindigkeit ("Hektik") +
               deren jeweils eigene Farben (clip-Fleck bleibt fix rot, kein *, s. LevelMeter.js). -->
          <div class="kme-row" data-f="showScale">
            <label>6dB-Skala</label>
            <input type="checkbox" class="es-showscale" title="Kleine -6/-12/…-Beschriftung neben dem Balken" />
          </div>
          <div class="kme-row" data-f="showRms">
            <label>RMS</label>
            <input type="checkbox" class="es-showrms" title="Zusätzliche dünne Linie auf Höhe des RMS-Pegels (der Balken selbst zeigt Peak)" />
          </div>
          <div class="kme-row" data-f="peakWidth">
            <label>Peak-Breite</label>
            <input type="number" class="es-peakwidth" min="1" max="2" step="1" title="Dicke der weißen Peak-Hold-Linie (1 oder 2px)" />
          </div>
          <div class="kme-row" data-f="peakHoldMs">
            <label>Peak-Hold</label>
            <input type="number" class="es-peakholdms" min="0" max="1000000" step="50" title="Wie lange die Peak-Hold-Linie auf dem letzten Maximum stehen bleibt, bevor sie abfällt (ms)" />
          </div>
          <div class="kme-row" data-f="hektikMs">
            <label>Hektik</label>
            <input type="number" class="es-hektikms" min="0" max="1000000" step="10" title="Reaktionsgeschwindigkeit des Balkens (ms) — kleiner = hektischer/direkter, größer = ruhiger/gedämpfter" />
          </div>
          <div class="kme-row" data-f="stepGrad">
            <label>6dB-Stufen</label>
            <input type="checkbox" class="es-stepgrad" title="Farbverlauf in sichtbaren 6dB-Stufen statt weich (nur ohne eigene Balkenfarbe wirksam)" />
          </div>
          <div class="kme-row" data-f="showLimiter">
            <label>Limiter</label>
            <input type="checkbox" class="es-showlimiter" title="Zweiter, rückwärts (von oben) füllender Balken: aktuelle Gain-Reduktion des Master-Limiters" />
          </div>
          <div class="kme-row" data-graphic="1" data-f="holdFg">
            <label>Peak-Farbe</label>
            <input type="color" class="es-holdfg" value="#ffffff" title="Farbe der Peak-Hold-Linie" />
            <button class="es-holdfg-clear kme-x" title="zurück zum Standard">✕</button>
          </div>
          <div class="kme-row" data-graphic="1" data-f="rmsFg">
            <label>RMS-Farbe</label>
            <input type="color" class="es-rmsfg" value="#8fd3ff" title="Farbe der RMS-Linie" />
            <button class="es-rmsfg-clear kme-x" title="zurück zum Standard">✕</button>
          </div>
          <div class="kme-row" data-graphic="1" data-f="scaleFg">
            <label>Skala-Farbe</label>
            <input type="color" class="es-scalefg" value="#8a93a3" title="Farbe der dB-Beschriftung/Striche" />
            <button class="es-scalefg-clear kme-x" title="zurück zum Standard">✕</button>
          </div>
          <div class="kme-row" data-graphic="1" data-f="limFg">
            <label>Limiter-Farbe</label>
            <input type="color" class="es-limfg" value="#ff5a5a" title="Farbe des Limiter-Reduktionsbalkens" />
            <button class="es-limfg-clear kme-x" title="zurück zum Standard">✕</button>
          </div>
          ...
          <!-- Signal-Scope – ALLE scope-eigenen Einstellungen (@dpa ddw.md 20260802, Punkt 3:
               „Scope selbst hat in Settings ja eigentlich nur Frame/Sample Umschalter, alles
               andere ist in Scope-Gruppen-Settings … auf dem Scope NUR die vollständigen
               Scope-Einstellungen, in der Gruppe nur die Standard-Gruppen-Settings" — Buffer/
               min/max/Auto-Range/Meter/Kurve zogen HIER aus dem gruppenspezifischen
               '_scopeKindSettings'-Hook (overcord/werkbank.js·werkbank-leer.js) um, Breite/Höhe
               nutzen die generischen boxSize/boxH-Felder (s. '_fieldsFor'). Die Gruppen-
               Settings behalten nur noch die Instanz-Aktionen (+➚/🚮/⟲, s. dort) — keine
               Style-Felder mehr, kein Überlapp. multiScope.js mappt boxSize/boxH→width/height. -->
          <div class="kme-row" data-f="bufferMs">
            <label>Buffer ms</label>
            <input type="number" class="es-bufferms" min="2" max="1000000" step="1" title="Zeitfenster des Ringpuffers (ms) — klemmt intern an der Web-Audio-FFT-Grenze" />
          </div>
          <div class="kme-row" data-f="scopeMin">
            <label>min</label>
            <input type="number" class="es-scopemin" step="0.1" title="Untere Wertgrenze (bei Auto-Range nur der Startwert)" />
          </div>
          <div class="kme-row" data-f="scopeMax">
            <label>max</label>
            <input type="number" class="es-scopemax" step="0.1" title="Obere Wertgrenze (bei Auto-Range nur der Startwert)" />
          </div>
          <div class="kme-row" data-f="autoRange">
            <label>Auto-Range</label>
            <input type="checkbox" class="es-autorange" title="min/max automatisch mitziehen (Steuersignale haben ganz verschiedene Bereiche)" />
          </div>
          <div class="kme-row" data-f="showMeter">
            <label>Meter</label>
            <input type="checkbox" class="es-showmeter" title="Meter-Säule (aktueller Wert) rechts neben der Kurve" />
          </div>
          <div class="kme-row" data-f="showCurve">
            <label>Kurve</label>
            <input type="checkbox" class="es-showcurve" title="Zeitverlauf als Kurve zeichnen" />
          </div>
    ...
    // Panel-Umschalter (@dpa dd.md 20260801): kippt NUR den Zustand, schließt das Fenster
    // NICHT (@dpa wörtlich: "die settings dabei nicht schließen, erst durch escape") - GroupHost
    // blendet über onOffPanel das Panel-Control aus/ein, wir bleiben offen und zeigen den
    // neuen Zustand direkt (Icon/Hilfstext/Ausgrauen).
    panel.querySelector('.kme-panelbtn').addEventListener('click', () => {
      if (!this._target) return;
      const next = !this.offPanelGet(this._target.id);
      this.onOffPanel(this._target.id, next);
      this._syncPanelBtn();
    });
    ...
    const live = ['.es-label', '.es-labelon', '.es-labelpos', '.es-btnmode', '.es-size', '.es-fontsize', '.es-boxsize', '.es-boxh', '.es-gap', '.es-beatsize', '.es-radius', '.es-pad', '.es-texton', '.es-textoff', '.es-textblink', '.es-memcols', '.es-memrows', '.es-slotsize', '.es-seqoff', '.es-seqmin', '.es-seqmax', '.es-seqstep', '.es-accuracy', '.es-showscale', '.es-showrms', '.es-peakwidth', '.es-peakholdms', '.es-hektikms', '.es-stepgrad', '.es-showlimiter', '.es-bufferms', '.es-scopemin', '.es-scopemax', '.es-autorange', '.es-showmeter', '.es-showcurve'];
    live.forEach((sel) => {
    ...
    // LevelMeter-Farben (@dpa dd.md 20260802): gleiches Flag-Muster wie oben.
    panel.querySelector('.es-holdfg').addEventListener('input', () => { this._holdFgOn = true; this._apply(); });
    panel.querySelector('.es-holdfg-clear').addEventListener('click', () => { this._holdFgOn = false; this._apply(); });
    panel.querySelector('.es-rmsfg').addEventListener('input', () => { this._rmsFgOn = true; this._apply(); });
    panel.querySelector('.es-rmsfg-clear').addEventListener('click', () => { this._rmsFgOn = false; this._apply(); });
    panel.querySelector('.es-scalefg').addEventListener('input', () => { this._scaleFgOn = true; this._apply(); });
    panel.querySelector('.es-scalefg-clear').addEventListener('click', () => { this._scaleFgOn = false; this._apply(); });
    panel.querySelector('.es-limfg').addEventListener('input', () => { this._limFgOn = true; this._apply(); });
    panel.querySelector('.es-limfg-clear').addEventListener('click', () => { this._limFgOn = false; this._apply(); });
  ...
  _fieldsFor(type, hasBlink) {
    ...
    // LevelMeter (@dpa 20260722, erweitert 20260802 dd.md): Balkenfarbe + Größe wie bisher,
    // dazu Skala/RMS-Linie/Peak-Breite+Haltezeit/6dB-Stufen/Limiter-Anzeige/Hektik + deren
    // Farben — kein Label (die Balken-Zonen sprechen für sich), keine Struktur-Felder.
    if (type === 'levelmeter') return ['fg', 'bg', 'boxSize', 'boxH', 'showScale', 'showRms', 'peakWidth', 'peakHoldMs', 'hektikMs', 'stepGrad', 'showLimiter', 'holdFg', 'rmsFg', 'scaleFg', 'limFg'];
    ...
    // Signal-Scope (ddw.md 20260727 + 20260802 Punkt 3): ALLE scope-eigenen Einstellungen
    // sitzen jetzt hier (statt verstreut über Scope-Settings UND Scope-Gruppen-Settings) —
    // Breite/Höhe über die generischen boxSize/boxH-Felder (multiScope.js mappt sie auf
    // SignalScope.style.width/height), Buffer/min/max/Auto-Range/Meter/Kurve neu, 'accuracy'
    // der bestehende frame/sample-Toggle. Farben (bg/curveColor/meterColor/gridColor) haben
    // weiterhin KEINE UI hier (vorbestehende Lücke, nicht Teil dieser Änderung).
    if (type === 'scope') return ['label', 'boxSize', 'boxH', 'bufferMs', 'scopeMin', 'scopeMax', 'autoRange', 'showMeter', 'showCurve', 'accuracy'];
    return ['label'];
  }
  ...
  open(target) {
    ...
    // Scope-Altbestand (vor 20260802 Punkt 3, s. unten): Breite/Höhe lagen dort unter
    // 'width'/'height' (Gruppen-Settings-Hook, andere Feldnamen als das generische
    // boxSize/boxH hier) — als Fallback lesen, damit ein bereits vom User gesetztes Maß
    // beim Umzug ins Scope-eigene Panel nicht verloren geht.
    this._panel.querySelector('.es-boxsize').value = style.boxSize || (target.type === 'scope' ? style.width : '') || '';
    this._panel.querySelector('.es-boxh').value = style.boxH || (target.type === 'scope' ? style.height : '') || '';
    ...
    // LevelMeter-Felder (@dpa dd.md 20260802): Skala/Limiter default AN, RMS/6dB-Stufen
    // default AUS (bewusst opt-in, s. LevelMeter.js-Kopf), Zeiten mit denselben Defaults
    // wie bisher fest im Code (Peak-Hold 800ms, Hektik 60ms).
    this._panel.querySelector('.es-showscale').checked = style.showScale !== false;
    this._panel.querySelector('.es-showrms').checked = !!style.showRms;
    this._panel.querySelector('.es-peakwidth').value = style.peakWidth || 1;
    this._panel.querySelector('.es-peakholdms').value = style.peakHoldMs ?? 800;
    this._panel.querySelector('.es-hektikms').value = style.hektikMs ?? 60;
    this._panel.querySelector('.es-stepgrad').checked = !!style.stepGrad;
    this._panel.querySelector('.es-showlimiter').checked = style.showLimiter !== false;
    this._holdFgOn = !!style.holdFg;
    this._panel.querySelector('.es-holdfg').value = style.holdFg || '#ffffff';
    this._rmsFgOn = !!style.rmsFg;
    this._panel.querySelector('.es-rmsfg').value = style.rmsFg || '#8fd3ff';
    this._scaleFgOn = !!style.scaleFg;
    this._panel.querySelector('.es-scalefg').value = style.scaleFg || '#8a93a3';
    this._limFgOn = !!style.limFg;
    this._panel.querySelector('.es-limfg').value = style.limFg || '#ff5a5a';
    // Signal-Scope – Buffer/min/max/Auto-Range/Meter/Kurve (@dpa ddw.md 20260802 Punkt 3,
    // umgezogen aus dem Scope-Gruppen-Settings-Hook): Defaults spiegeln SignalScope.js'
    // SCOPE_DEFAULTS 1:1 (bufferMs 40, min 0, max 1, autoRange/showMeter/showCurve an).
    this._panel.querySelector('.es-bufferms').value = style.bufferMs ?? 40;
    this._panel.querySelector('.es-scopemin').value = style.minVal ?? 0;
    this._panel.querySelector('.es-scopemax').value = style.maxVal ?? 1;
    this._panel.querySelector('.es-autorange').checked = style.autoRange !== false;
    this._panel.querySelector('.es-showmeter').checked = style.showMeter !== false;
    this._panel.querySelector('.es-showcurve').checked = style.showCurve !== false;
    ...
    // eine knappe Pixel-Decke, großzügig statt "zur Sicherheit" eng, überall.
    const gapIn = this._panel.querySelector('.es-gap');
    if (gapIn) gapIn.max = 1000000;
    ...
    if (target.type === 'keyboard') {
      ...
      wIn.min = 10; wIn.max = 1000000; wIn.step = 1; hint(wIn, 'Breite EINER Taste (px)');
      hIn.min = 10; hIn.max = 1000000; hIn.step = 1; hint(hIn, 'Höhe EINER Taste (px)');
    } else {
      if (hLab) hLab.textContent = 'Höhe';
      if (fgLab) fgLab.textContent = 'Text';
      wIn.min = 20; wIn.max = 1000000; wIn.step = 2; hint(wIn, 'Breite des Feldes (px)');
      hIn.min = 16; hIn.max = 1000000; hIn.step = 2; hint(hIn, 'Höhe des Feldes (px)');
    }
  ...
  /** Panel-Umschalter (@dpa dd.md 20260801): Icon + `.on`-Klasse + Hilfstext aus dem
   *  echten Zustand (offPanelGet, von GroupHost gesetzt) nachziehen - UND die rein
   *  grafischen Zeilen ausgrauen, weil sie bedeutungslos sind, solange das Control gar
   *  nicht auf dem Panel sitzt (Tontechnik-Felder bleiben immer bedienbar, s. Kopf-
   *  Kommentar der Datei). Rein optisch (CSS-Klasse), die Felder bleiben bedienbar,
   *  damit man das Aussehen für die Rückkehr aufs Panel schon vorbereiten kann. */
  _syncPanelBtn() {
    if (!this._target) return;
    const btn = this._panel.querySelector('.kme-panelbtn');
    const off = this.offPanelGet(this._target.id);
    btn.innerHTML = '';
    btn.appendChild(icon(off ? 'panelOut' : 'panelIn'));
    ...
    hint(btn, txt);
    this._panel.classList.toggle('kme-dim-graphics', off);
  }
  ...
  _collect() {
    ...
    // WICHTIG: KEIN .trim() — Leerzeichen sind Teil des User-sichtbaren Labels und dürfen nicht entfernt werden
    if (fields.has('textOn')) { const v = P('.es-texton').value; if (v) s.textOn = v; }
    if (fields.has('textOff')) { const v = P('.es-textoff').value; if (v) s.textOff = v; }
    if (fields.has('textBlink')) { const v = P('.es-textblink').value; if (v) s.textBlink = v; }
    ...
    // WICHTIG: KEIN .trim() — Leerzeichen sind absichtlich und sollen erhalten bleiben
    if (fields.has('optLabels') && this._optInputs && this._optInputs.length) {
        const arr = this._optInputs.map((i) => i.value);
    ...
    // LevelMeter (@dpa dd.md 20260802): Checkboxen definitiv (wie labelOn/seqOff), Zeiten/
    // Breite geklemmt, Farben nur mit Flag (gleiches Muster wie mainColor/vg1 oben).
    if (fields.has('showScale')) s.showScale = P('.es-showscale').checked;
    if (fields.has('showRms')) s.showRms = P('.es-showrms').checked;
    if (fields.has('peakWidth')) { const v = parseInt(P('.es-peakwidth').value); if (v) s.peakWidth = Math.max(1, Math.min(2, v)); }
    if (fields.has('peakHoldMs')) { const v = parseInt(P('.es-peakholdms').value); if (Number.isFinite(v)) s.peakHoldMs = Math.max(0, v); }
    if (fields.has('hektikMs')) { const v = parseInt(P('.es-hektikms').value); if (Number.isFinite(v)) s.hektikMs = Math.max(0, v); }
    if (fields.has('stepGrad')) s.stepGrad = P('.es-stepgrad').checked;
    if (fields.has('showLimiter')) s.showLimiter = P('.es-showlimiter').checked;
    if (fields.has('holdFg') && this._holdFgOn) s.holdFg = P('.es-holdfg').value;
    if (fields.has('rmsFg') && this._rmsFgOn) s.rmsFg = P('.es-rmsfg').value;
    if (fields.has('scaleFg') && this._scaleFgOn) s.scaleFg = P('.es-scalefg').value;
    if (fields.has('limFg') && this._limFgOn) s.limFg = P('.es-limfg').value;
    ...
    // Signal-Scope (@dpa ddw.md 20260802 Punkt 3): Buffer/min/max definitiv (wie labelOn/
    // seqOff), damit ein einziges Feld-Ändern nie den Rest des Scope-Stils stillschweigend
    // auf die Defaults zurückwirft (das WAR der „Frame/Sample minifiziert"-Bug, s.
    // multiScope.js) — jede _apply() schreibt jetzt den KOMPLETTEN Scope-Stil zurück.
    if (fields.has('bufferMs')) { const v = parseFloat(P('.es-bufferms').value); if (Number.isFinite(v)) s.bufferMs = Math.max(2, v); }
    if (fields.has('scopeMin')) { const v = parseFloat(P('.es-scopemin').value); if (Number.isFinite(v)) s.minVal = v; }
    if (fields.has('scopeMax')) { const v = parseFloat(P('.es-scopemax').value); if (Number.isFinite(v)) s.maxVal = v; }
    if (fields.has('autoRange')) s.autoRange = P('.es-autorange').checked;
    if (fields.has('showMeter')) s.showMeter = P('.es-showmeter').checked;
    if (fields.has('showCurve')) s.showCurve = P('.es-showcurve').checked;
    return s;
  }
