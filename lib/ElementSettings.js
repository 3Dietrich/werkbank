...
export class ElementSettings {
  ...
  _build() {
    ...
    panel.innerHTML = `
      ...
      <div class="kme-body">
        ...
        <div class="kme-grid">
          ...
          <!-- Button-Modus (@dpa 20260719): Trigger=Impuls (Farbe fadet an→aus), Gate=an
               solange gedrückt, Umschalter=an/aus je Klick. Default kommt aus den defs. -->
          <div class="kme-row" data-f="btnMode">
            <label>Modus</label>
            <select class="es-btnmode" title="Trigger (Impuls) · Gate (solange gedrückt) · Umschalter (an/aus)">
        ...
        <!-- Wechsel-Button (@dpa 20260722_194404): eine Tabelle, eine Zeile pro Stufe (n),
             umsortierbar (▲▼) — Caption/Farbe/Kurzbeschreibung je Stufe. n verändert die
             Zeilenzahl (mehr = neue Default-Zeile ans Ende, weniger = letzte Zeile weg). -->
        <div class="kme-row kme-wide es-wechsel-row" data-f="wechselTable">
          <label>n <input type="number" class="es-wechsel-n" min="1" max="12" step="1" /></label>
    ...
    const live = ['.es-label', '.es-labelon', '.es-labelpos', '.es-btnmode', '.es-size', '.es-fontsize', '.es-boxsize', '.es-boxh', '.es-gap', '.es-beatsize', '.es-radius', '.es-pad', '.es-texton', '.es-textoff', '.es-textblink', '.es-memcols', '.es-memrows', '.es-slotsize'];
    live.forEach((sel) => {
  ...
  _fieldsFor(type, hasBlink) {
    ...
    // Wechsel-Button (@dpa 20260722_194404): NUR die Stufen-Tabelle — kein Label/Farben/
    // Größe außenrum, das steckt alles schon je Stufe in der Tabelle selbst.
    if (type === 'wechsel') return ['wechselTable'];
    return ['label'];
  }
  ...
  open(target) {
    ...
    esLabel.value = style.label ?? (target.defLabel || '');
  ...
  /** Wechsel-Button-Tabelle (@dpa 20260722_194404): eine Zeile pro Stufe — ▲▼ (umsortieren),
   *  Caption, Farbe, Kurzbeschreibung, ✕ (entfernen). `n` (Zahlenfeld oben) fügt am Ende neue
   *  Default-Zeilen an bzw. entfernt die letzten. defModes (target.defModes) sind die vom
   *  Aufrufer/„KI" vorgefertigten Stufen fürs Erstbefüllen (@dpa: „KI-vorgefertigte aber
   *  editierbare Kurzbeschreibung"); ein gespeicherter Style (style.modes) überschreibt sie. */
  _buildWechselTable(target, style) {
    const nInput = this._panel.querySelector('.es-wechsel-n');
    ...
    const render = () => {
      ...
      modes.forEach((m, i) => {
        ...
        const desc = document.createElement('input'); desc.type = 'text'; desc.className = 'es-wechsel-desc';
        desc.value = m.desc || ''; desc.placeholder = 'Kurzbeschreibung'; hint(desc, 'Erklärt diese Stufe im Mouseover-Hint des Knopfs');
        ...
        rm.addEventListener('click', () => { modes.splice(i, 1); nInput.value = modes.length; render(); this._apply(); });
    ...
    render();
    nInput.onchange = () => {
      const n = Math.max(1, Math.min(12, nInput.valueAsNumber | 0 || 1));
      while (modes.length < n) modes.push({ caption: 'Modus ' + (modes.length + 1), color: '', desc: '' });
      while (modes.length > n) modes.pop();
      render(); this._apply();
    };
  }
