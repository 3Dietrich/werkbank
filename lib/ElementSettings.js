...
const DESIGN_TEXT_FIELDS = new Set(['label', 'labelOn', 'textOn', 'textOff', 'optLabels']);
...
export class ElementSettings {
  ...
  _build() {
    ...
    const live = ['.es-label', '.es-labelon', '.es-labelpos', '.es-btnmode', '.es-size', '.es-fontsize', '.es-boxsize', '.es-boxh', '.es-gap', '.es-beatsize', '.es-radius', '.es-pad', '.es-texton', '.es-textoff', '.es-memcols', '.es-memrows', '.es-slotsize'];
    live.forEach((sel) => {
  ...
  _fieldsFor(type) {
    ...
    if (type === 'button') return ['label', 'labelPos', 'btnMode', 'textOn', 'textOff', 'size', 'fg', 'pad', 'bg', 'bgOn', 'boxSize', 'boxH'];
