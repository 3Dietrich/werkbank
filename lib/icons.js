...
const PATHS = {
    ...
    // ✎ Bearbeiten (@dpa 20260723_140151: „[edit symbol]" — Sq hinzufügen/entfernen). Stift
    // diagonal, Spitze unten links; Radiergummi-frei, damit es nicht mit `trash` verwechselt wird.
    edit: '<path d="M16.4 3.4a2.1 2.1 0 0 1 3 3L7 18.8l-4 1.2 1.2-4z"/><path d="M14.8 5l3.2 3.2"/>',
    // ⓘ Instrument-Info (@dpa 20260722_130710, ddw.md image-17) – Kreis + „i". Bewusst
    // eigenes Icon statt `help` (Fragezeichen): unterschiedliche Bedeutung (Beschreibung
    // ANZEIGEN vs. Hilfe-Blasen an/aus), gleicher Fehler wie beim Zahnrad wäre sonst
    // möglich – der i-Punkt ist deshalb ebenfalls gefüllt, nicht gestrichelt.
    info: '<circle cx="12" cy="12" r="10.4"/><path d="M12 11v6.4"/>'
        + '<circle cx="12" cy="7.2" r="1.3" fill="currentColor" stroke="none"/>',
    // ⬜ Panel-Umschalter „an" (@dpa dd.md 20260801: eigener Toggle "Panel?" - NICHTS mit der
    // Knob-Gestalt "Ohne" zu tun, das bleibt AUF dem Panel, hier geht es ums OB überhaupt).
    // Zustand "auf dem Panel": ein gefülltes Element SITZT MITTIG in einem umrandeten Panel-
    // Rechteck - ruhiges, geschlossenes Bild, kein Pfeil, nichts bewegt sich.
    panelIn: '<rect x="2.4" y="3.6" width="19.2" height="16.8" rx="2"/>'
        + '<rect x="8.8" y="8.8" width="6.4" height="6.4" rx="1.2" fill="currentColor" stroke="none"/>',
    // ⬜→ Panel-Umschalter „aus" (Control NICHT auf dem Panel, sondern nur in den Gruppen-
    // Settings). Bewusst eine andere KOMPOSITION statt nur eine gespiegelte Kleinigkeit, damit
    // der Unterschied auch auf ~14px sofort auffällt (@dpa: "eindeutiges Icon"): das Panel-
    // Rechteck ist schmaler, ein Pfeil zeigt nach draußen, das gefüllte Element sitzt
    // AUSSERHALB - dasselbe Bild wie panelIn, aber sichtbar "ausgezogen".
    panelOut: '<rect x="1.6" y="3.6" width="14.4" height="16.8" rx="2"/>'
        + '<path d="M7.6 12h8.8M13 8l4 4-4 4"/>'
        + '<rect x="18.4" y="9.4" width="5" height="5.2" rx="1" fill="currentColor" stroke="none"/>',
};
