...
import { lang, text as i18nText, hint } from './i18n.js';
...
export class KnobMetaEditor {
  ...
  _build() {
    ...
    panel.innerHTML = `
      <div class="kme-header">
        ...
        <!-- Panel-Umschalter (@dpa dd.md 20260801): "es braucht einen völlig neuen toggle
             für 'Panel?'" - eigenständig, NICHTS mit der Gestalt "Ohne" zu tun (die bleibt
             auf dem Panel, nur ohne Dial). Icon+Hilfstext synchronisiert open(). -->
        <button class="kme-panelbtn" type="button"></button>
        <button class="kme-close" title="Schließen">✕</button>
      </div>
      <div class="kme-body">
        <!-- Design-Speicher (@dpa 20260722_013727, ddw.md): ganzes Aussehen (ohne Namen/
             Hilfe) als Preset merken/abrufen — für andere Regler. Dieselbe designPresets-
             Ablage wie ElementSettings.js, Bucket „knob". -->
        <div class="kme-row kme-wide">
          <label>Design</label>
          <div class="kme-design-menu"></div>
        </div>
        <div class="kme-row kme-wide">
          <label>Name</label>
        ...
        <div class="kme-grid">
          ...
          <div class="kme-row">
            <label>Skew</label>
            <input type="number" class="kme-skew" min="0.01" max="1000000" step="0.1" value="1" title="Verzieht die lineare Kurve: &lt;1 spreizt unten, &gt;1 spreizt oben (1 = unverzogen)" />
          </div>
          ...
          <!-- Wert-Schriftgröße + Padding (@dpa 20260718_203341): der ANGEZEIGTE Wert ist bei
               ...
               Default aus dem CSS. -->
          <div class="kme-row" data-graphic="1">
            <label title="Schriftgröße des angezeigten Werts">Textgr.</label>
            <input type="number" class="kme-valuefont" min="7" max="1000000" step="1" title="Schriftgröße der Wert-Anzeige (px, leer = Standard)" />
          </div>
          <div class="kme-row" data-graphic="1">
            <label title="Innenabstand des Reglers">Padding</label>
            <input type="number" class="kme-pad" min="0" max="1000000" step="1" title="Innenabstand um den Regler (px, leer = Standard)" />
          </div>
          <div class="kme-row" data-graphic="1">
            <label>Label</label>
          ...
          <div class="kme-row" data-graphic="1" data-f="view">
            <label>Größe</label>
          ...
          <div class="kme-row" data-graphic="1" data-f="faderLen">
            <label>Länge</label>
            <input type="number" class="kme-faderlen" min="24" max="1000000" step="4" title="Länge der Fader-Bahn in px" />
          </div>
        </div>
        ...
        <div class="kme-row kme-wide kme-color-row" data-graphic="1">
          <label>Farben</label>
        ...
        <div class="kme-curve-preview">
          <div class="kme-keyhint"><b>Enter</b> = <span class="kme-kh-apply"></span><br><b>ESC</b> = <span class="kme-kh-leave"></span></div>
    ...
    panel.querySelector('.kme-panelbtn').addEventListener('click', () => {
      ...
      if (!id) return;
      const next = !this.offPanelGet(id);
      this.onOffPanel(id, next);
      this._syncPanelBtn();
    });
    i18nText(panel.querySelector('.kme-kh-apply'), 'Übernehmen');
    i18nText(panel.querySelector('.kme-kh-leave'), 'Verlassen');
    ...
    // Label sofort anwenden (@dpa 20260724_122929: "die Leerzeichen sind immer wieder weg") —
    // bisher gab es HIER keinen Live-Listener (anders als bei fast jedem anderen Feld unten),
    // die Eingabe griff nur bei Enter oder zufällig, wenn parallel ein anderes Feld _apply()
    // auslöste. Ohne Enter ging jede Label-Änderung beim Schließen (ESC/Außenklick/✕) verloren.
    panel.querySelector('.kme-label').addEventListener('input', () => this._apply());
    ...
    // Range (Min/Max/Step/Dez./Einheit) sofort anwenden (ddw.md 20260727_135331, @dpa: "alle
    // Einstellungsveränderungen sollen DIREKT übernommen werden, ohne Enter drücken zu müssen.
    // Enter nur noch für den User zur gewohnten Sicherheit") — bisher die einzigen Felder
    // dieses Panels ganz OHNE Live-Listener, griffen nur bei Enter oder zufällig als
    // Nebeneffekt eines anderen Feld-Listeners (dasselbe Muster, das schon bei .kme-label
    // oben gefixt wurde).
    panel.querySelector('.kme-min').addEventListener('input', () => this._apply());
    panel.querySelector('.kme-max').addEventListener('input', () => this._apply());
    panel.querySelector('.kme-step').addEventListener('input', () => this._apply());
    panel.querySelector('.kme-decimals').addEventListener('input', () => this._apply());
    panel.querySelector('.kme-unit').addEventListener('input', () => this._apply());
    ...
    // Design-Presets als PickMenu (@dpa 20260722_013727) – dasselbe designPresets-Objekt
    // wie ElementSettings.js (geteilter State pro Instrument), Bucket „knob".
    this._designSel = '';   // zuletzt gewählter Design-Name (nur UI-Hervorhebung, nicht persistiert)
    this._designMenu = new PickMenu({
      empty: '— keins —',
      title: 'Gespeichertes Design wählen (nur fürs Aussehen – Name/Hilfe bleiben unberührt)',
      list: () => this._designPresets(),
      current: () => this._designSel,
      onPick: (i, p) => { this._designSel = p.name; this._applyDesign(p.style); },
      onUpdate: (i) => {
        if (!this._state) return;
        const list = this._designPresets().slice();
        list[i] = { ...list[i], style: this._currentDesignStyle() };
        this._state.set('designPresets', { ...(this._state.get('designPresets') || {}), knob: list });
      },
      onRename: (i, p, nm) => {
        if (!this._state) return '';
        const list = this._designPresets().slice();
        ...
        this._state.set('designPresets', { ...(this._state.get('designPresets') || {}), knob: list });
        if (this._designSel === p.name) this._designSel = nm;
        return '';
      },
      onDelete: (i, p) => {
        if (!this._state || !confirm('Design „' + p.name + '" löschen?')) return;
        const list = this._designPresets().slice(); list.splice(i, 1);
        this._state.set('designPresets', { ...(this._state.get('designPresets') || {}), knob: list });
        if (this._designSel === p.name) this._designSel = '';
      },
      foot: [
        ['plus', 'Neu…', 'Aktuelles Aussehen (ohne Name/Hilfe) als Design speichern', () => this._saveDesignPreset()],
      ],
    });
    panel.querySelector('.kme-design-menu').appendChild(this._designMenu.element);
    ...
    makeDraggable(panel, panel.querySelector('.kme-header'), (pos) => {
      if (this._state) this._state.set('knobSettingsPos', pos);
    });
    ...
    // DREI Ausnahmen, alle sind Bedienung DIESES Panels, nur außerhalb seines DOM:
    //  • das Farb-PickMenu (Popup an <body>, s. PickMenu.contains),
    //  • das Design-PickMenu (dieselbe Bauart, @dpa 20260722_013727) – ohne diese Ausnahme
    //    schloss der mousedown-Klick auf „Neu…"/eine Design-Zeile das Panel VOR dem Klick-
    //    Handler und _saveDesignPreset() griff ins Leere (this._currentKnob war schon null;
    //    exakt der historische Bug „Farbe nicht speicherbar", @dpa 20260716_174111),
  ...
  open(knob) {
    ...
    // Gemerkte Position (ddw.md 20260726) schlägt die Anker-Berechnung neben dem Regler.
    const savedPos = this._state && this._state.get('knobSettingsPos');
    if (savedPos) {
   ...
   *  grafischen Zeilen ausgrauen, weil sie bedeutungslos sind, solange dieser Regler gar
   *  nicht auf dem Panel sitzt (Tontechnik-Felder wie Min/Max/Kurve bleiben immer
   *  bedienbar, s. Kopf-Kommentar der Datei). Rein optisch (CSS-Klasse), die Felder
   *  bleiben bedienbar, damit man das Aussehen für die Rückkehr aufs Panel schon
   *  vorbereiten kann. */
  _syncPanelBtn() {
    const id = this._hintId();
    if (!id) return;
    const btn = this._panel.querySelector('.kme-panelbtn');
    const off = this.offPanelGet(id);
    btn.innerHTML = '';
  ...
  _apply() {
    ...
    // Platzhalter-Trick ("/  " soll breiter wirken als "/") — nur eine wirklich LEERE
    // Eingabe (Länge 0) fällt auf das bisherige Label zurück.
    const labelRaw = this._panel.querySelector('.kme-label').value;
    // Min/Max sind die Range selbst, kein „leer = Standard"-Feld wie Default/Pad (@dpa
    // 20260727: Höhen-Dämpf-Regression — ein geleertes Max-Feld fiel bisher live auf den
    // hartverdrahteten Knob.js-Fallback (max||1) zurück und deckelte den Regler SOFORT auf
    // 1, statt einfach nichts zu tun. Bei leerem/ungültigem Feld bleibt der bisherige
    // Meta-Wert unangetastet — kein stiller Reset auf 0/1.
    const meta0 = this._currentKnob.getMeta();
    const minRaw = parseFloat(this._panel.querySelector('.kme-min').value);
    const maxRaw = parseFloat(this._panel.querySelector('.kme-max').value);
    this._currentKnob.setMeta({
      label: labelRaw.length ? labelRaw : this._currentKnob.label,
      min: Number.isFinite(minRaw) ? minRaw : meta0.min,
      max: Number.isFinite(maxRaw) ? maxRaw : meta0.max,
  ...
  /* ── Design-Presets (@dpa 20260722_013727) ── */

  _designPresets() { return (this._state && (this._state.get('designPresets') || {}).knob) || []; }

  /** NUR die optischen Felder – min/max/step/curve/skew/unit/decimals/defaultValue/
   *  dragRange gehören zur FUNKTION dieses einen Reglers (was er überhaupt regelt), nicht
   *  zu seinem Aussehen. Ein „Design" darf einem Hz-Regler nicht heimlich einen dB-Bereich
   *  unterschieben. */
  _currentDesignStyle() {
    const meta = this._currentKnob.getMeta();
    const fields = ['viewSize', 'color', 'labelPos', 'bg', 'shape', 'faderLen', 'valueFontSize', 'pad', 'hideValue'];
    const out = {};
    fields.forEach((k) => { if (meta[k] !== undefined) out[k] = meta[k]; });
    return out;
  }

  /** Ein Design auf den offenen Regler anwenden: mit dem aktuellen Meta mergen (Funktions-/
   *  Text-Felder bleiben unberührt, ein Design enthält nie welche), persistieren, Panel aus
   *  dem neuen Stand neu aufbauen (open() liest ohnehin aus knob.getMeta()). */
  _applyDesign(designStyle) {
    if (!this._currentKnob) return;
    this._currentKnob.setMeta(designStyle);
    if (this.onApply) this.onApply(this._currentKnob);
    this.open(this._currentKnob);
  }

  _saveDesignPreset() {
    if (!this._state || !this._currentKnob) return;
