...
/**
 * Demo-/Erstbesuch-Datei dieses Einstiegs — als absolute URL, aufgelöst gegen den ORT
 * DIESES MODULS statt gegen die aufrufende Seite.
 *
 * Wichtig, seit Einstiege in Unterordnern liegen (`/werkbank-leer/`, @dpa dd.md
 * 20260801_3): ein relativer Pfad wie `presets/x.json` wird gegen die Dokument-URL
 * aufgelöst, aus `/werkbank-leer/` würde also `/werkbank-leer/presets/x.json` — die
 * Datei läge dort nicht, und der Erstbesuch bliebe still ohne Demo-Stand. `import.meta.url`
 * zeigt immer auf `lib/appId.js`, `../presets/` davon ist also verlässlich der eine
 * presets-Ordner im Wurzelverzeichnis — egal, aus welcher Ordnertiefe die Seite lädt.
 */
export const configPath = () => new URL(`../presets/${APP}-config.json`, import.meta.url).href;
