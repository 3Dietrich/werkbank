 ...
 * @param {{name:string, instr:object}[]} [o.isms] – die „Standard"-ISMs dieses Einstiegs
 *        (Tempo&MM/Scope/Debug/Rec/Meter, ddw.md 20260803_135251), je mit ihrem
 *        `mountInstrumentSettings()`-Rückgabeobjekt (`isHidden()`/`setHidden()`/`name()`).
 *        Zeigt hier NUR die aktuell ausgeblendeten (sonst fände man ein weggeschaltetes ISM
 *        nirgends wieder, @dpa: „Muss aber 'bekannt' sein") — sichtbare stehen bereits auf
 *        dem Panel und brauchen keine zweite Fundstelle. Der Sichtbar-Umschalter SELBST sitzt
 *        in jedem ISM in dessen eigenen Settings (lib/InstrumentSettings.js, Rechtsklick auf
 *        den Instrument-Header) — genau wie bei Controls (GroupHost `ctrlOffPanel`). Optional,
 *        damit ein Einstieg ohne Standard-ISMs (z.B. ein künftiger reiner DSP-Baukasten) nicht
 *        crasht.
...
export function buildMainSettings(f, { state, onExport, onImport, onReset, onNewEntry, reopen, backups, isms, recState }) {
    ...
    // ISM-Sichtbarkeit (ddw.md 20260803_135251, s. JSDoc oben) — nur die Unterrubrik der
    // aktuell AUSGEBLENDETEN Standard-ISMs, sonst wären sie nach dem Wegschalten nirgends
    // mehr zu finden. Der Umschalter selbst (an/aus) sitzt in jedem ISM in dessen eigenen
    // Settings, s. lib/InstrumentSettings.js.
    if (isms && isms.length) {
        section('ISMs', 'nur dieser Einstieg', ISMS_INFO);
        const hiddenIsms = isms.filter((it) => it.instr && it.instr.isHidden && it.instr.isHidden());
        if (!hiddenIsms.length) {
