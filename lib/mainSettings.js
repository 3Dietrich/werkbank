...
import { isOutsourceMode } from './newEntryFlow.js';
import { MP3_CBR_PRESETS } from './mp3Encoder.js';
import { WAV_SAMPLE_RATES, WAV_BIT_DEPTHS } from './wavEncoder.js';

// Aufnahme-Format (ddw.md 20260803_135251 Punkt B, s. JSDoc bei o.recState unten) — dieselbe
// Werte-Liste, die vorher in overcord/werkbank.js UND werkbank-leer/werkbank-leer.js doppelt
// stand (Rec-Instrument-TODO 2, @dpa 20260721).
const REC_FORMATS = [
    { v: 'webm', l: 'WebM/Opus' },
    { v: 'mp3', l: 'MP3' },
    { v: 'wav', l: 'WAV' },
 ...
 * @param {{name:string, instr:object, noHeader?:boolean}[]} [o.isms] – die „Standard"-ISMs
 *        dieses Einstiegs (Tempo&MM/Scope/Debug/Rec/Meter, ddw.md 20260803_135251), je mit
 *        ihrem `mountInstrumentSettings()`-Rückgabeobjekt (`isHidden()`/`setHidden()`/`name()`).
 *        Zeigt für normale (geheaderte) ISMs NUR die aktuell ausgeblendeten (sonst fände man
 *        ein weggeschaltetes ISM nirgends wieder, @dpa: „Muss aber 'bekannt' sein") —
 *        sichtbare stehen bereits auf dem Panel und brauchen keine zweite Fundstelle. Der
 *        Sichtbar-Umschalter SELBST sitzt in jedem ISM in dessen eigenen Settings
 *        (lib/InstrumentSettings.js, Rechtsklick auf den Instrument-Header) — genau wie bei
 *        Controls (GroupHost `ctrlOffPanel`).
 *        `noHeader: true` (bisher nur Meter/LevelMeter, s. lib/LevelMeter.js-Kopf) markiert
 *        ISMs OHNE `h2` — die können den Rechtsklick-Weg gar nicht gehen (kein Header zum
 *        Draufklicken), darum steht ihre Zeile hier IMMER (nicht nur wenn schon ausgeblendet),
 *        mit einem einfachen Ein/Aus-Knopf statt „wieder einblenden" (@dpa dd.md 20260803:
 *        „beim 'Außenseiter' Label ist es zu sehen, dieses weg-Icon soll das LevelMeter in
 *        die main Settings wandern"). Optional, damit ein Einstieg ohne Standard-ISMs (z.B.
 *        ein künftiger reiner DSP-Baukasten) nicht crasht.
 * @param {{get:Function,set:Function}} [o.recState] – der State des Rec-Instruments
 *        (ddw.md 20260803_135251 Punkt B: @dpa wollte „Rec-Format weg vom Panel, rein in die
 *        globalen Settings" — vorher ein eigener Header-Knopf ⚙ Rec-Format mit Popover, jetzt
 *        hier). State-Keys unverändert: `recFormat`/`recMp3Bitrate`/`recMp3Stereo`/
 *        `recWavSampleRate`/`recWavBitDepth`. Optional, damit ein Einstieg ohne Rec-ISM nicht
 *        crasht.
 */
export function buildMainSettings(f, { state, onExport, onImport, onReset, onNewEntry, reopen, backups, isms, recState }) {
    const { section, num, select, color, colorA, full, raw } = f;
    ...
    // ISM-Sichtbarkeit (ddw.md 20260803_135251, s. JSDoc oben) — zwei Gruppen:
    //  - geheaderte ISMs: nur die Unterrubrik der aktuell AUSGEBLENDETEN, sonst wären sie
    //    nach dem Wegschalten nirgends mehr zu finden. Der Umschalter selbst (an/aus) sitzt
    //    in jedem ISM in dessen eigenen Settings, s. lib/InstrumentSettings.js.
    //  - `noHeader`-ISMs (bisher nur Meter/LevelMeter): haben GAR KEINEN Rechtsklick-Weg
    //    (kein h2), darum IMMER hier gelistet, mit einfachem Ein/Aus statt „wieder
    //    einblenden" (@dpa dd.md 20260803, s. JSDoc oben).
    if (isms && isms.length) {
        section('ISMs', 'nur dieser Einstieg', ISMS_INFO);
        const alwaysIsms = isms.filter((it) => it.noHeader && it.instr);
        const hiddenIsms = isms.filter((it) => !it.noHeader && it.instr && it.instr.isHidden && it.instr.isHidden());
        if (!alwaysIsms.length && !hiddenIsms.length) {
            // Handgebaut statt note()-Helfer (der übersetzt nicht, s. i18nText-Muster bei
            // newEntryNote/dataNote weiter unten in dieser Datei — dieselbe Zeilenoptik .sw-note).
            const p = document.createElement('div'); p.className = 'sw-note';
            i18nText(p, 'Aktuell ist kein Standard-Instrument ausgeblendet.');
            full(p);
        }
        // `noHeader`-ISMs: dauerhafte Zeile, Knopf spiegelt live den Zustand (Sync statt
        // Entfernen — die Zeile bleibt ja stehen, anders als bei den geheaderten unten).
        alwaysIsms.forEach((it) => {
            const btn = document.createElement('button');
            btn.type = 'button'; btn.className = 'kme-panelbtn';
            const sync = () => {
                const h = it.instr.isHidden();
                btn.innerHTML = '';
                btn.appendChild(icon(h ? 'panelOut' : 'panelIn'));
                btn.classList.toggle('on', h);
                const txt = h ? 'Ausgeblendet – klicken, um es wieder anzuzeigen.' : 'Sichtbar – klicken, um es auszublenden.';
                btn.setAttribute('aria-label', txt); hint(btn, txt);
            };
            btn.addEventListener('click', () => { it.instr.setHidden(!it.instr.isHidden()); sync(); });
            sync();
            raw(it.instr.name() || it.name, btn);
        });
        hiddenIsms.forEach((it) => {
            const btn = document.createElement('button');
            btn.type = 'button'; btn.className = 'kme-panelbtn on';
            btn.appendChild(icon('panelOut'));
            // Fester Text OHNE den Namen drin (@dpa-Regel: Hints als EIN Literal, eine
            // Verkettung 'a '+name+' b' verlöre die Übersetzung still) — der Name steht
            // schon als Zeilen-Label daneben (raw()), muss im Hint nicht wiederholt werden.
            const txt = 'Ausgeblendet – klicken, um es wieder anzuzeigen.';
            btn.setAttribute('aria-label', txt); hint(btn, txt);
            const r = raw(it.instr.name() || it.name, btn);
            // Reines Entfernen der Zeile reicht (kein Neuaufbau): das Panel dahinter zeigt
            // das ISM sofort wieder (State-Change läuft über subscribe in GroupHost/
            // InstrumentSettings selbst), die Liste hier muss nur diese eine Zeile los werden.
            btn.addEventListener('click', () => { it.instr.setHidden(false); r.remove(); });
        });
    }

    // Aufnahme-Format (ddw.md 20260803_135251 Punkt B, s. JSDoc bei o.recState) — vorher ein
    // eigener Header-Knopf mit Segment-Buttons/Popover, hier als normale select()-Zeilen wie
    // der Rest des Fensters (Sprache macht es genauso). MP3-/WAV-Unterzeilen nur sichtbar,
    // wenn das jeweilige Format aktiv ist — dieselbe Ein-/Ausblend-Logik wie vorher im Popover.
    if (recState) {
        section('Aufnahme-Format', 'nur dieser Einstieg', RECFORMAT_INFO);
        const fmtSel = select('Format', {
            options: REC_FORMATS.map((o) => [o.v, o.l]),
            get: () => recState.get('recFormat') || 'webm',
            set: (v) => { recState.set('recFormat', v); refreshRecVis(); },
            title: 'Aufnahme-Ausgabeformat (global, für alle Aufnahmen)',
        });
        // Nur CBR (@dpa-Entscheidung 20260721: VBR fehlt lamejs strukturell, s.
        // lib/vendor/lame.js); Qualität fest auf 3, kein UI-Feld dafür (1:1 aus dem alten Popover).
        const bitrateSel = select('Bitrate', {
            options: MP3_CBR_PRESETS.map((kbps) => [String(kbps), kbps + ' kbps']),
            get: () => String(recState.get('recMp3Bitrate') || 192),
            set: (v) => recState.set('recMp3Bitrate', parseInt(v, 10)),
            title: 'MP3-Bitrate (CBR)',
        });
        const chSel = select('Kanäle', {
            options: [['mono', 'Mono'], ['stereo', 'Stereo']],
            get: () => (recState.get('recMp3Stereo') !== false ? 'stereo' : 'mono'),
            set: (v) => recState.set('recMp3Stereo', v === 'stereo'),
            title: 'MP3-Kanäle',
        });
        // Resampling linear, kein Dithering (s. lib/wavEncoder.js).
        const rateSel = select('Samplerate', {
            options: WAV_SAMPLE_RATES.map((r) => [String(r), (r / 1000) + ' kHz']),
            get: () => String(recState.get('recWavSampleRate') || 44100),
            set: (v) => recState.set('recWavSampleRate', parseInt(v, 10)),
            title: 'WAV-Samplerate',
        });
        const bitSel = select('Bittiefe', {
            options: WAV_BIT_DEPTHS.map((bd) => [String(bd), bd + ' Bit']),
            get: () => String(recState.get('recWavBitDepth') || 16),
            set: (v) => recState.set('recWavBitDepth', parseInt(v, 10)),
            title: 'WAV-Bittiefe',
        });
        const refreshRecVis = () => {
            const c = recState.get('recFormat') || 'webm';
            const showMp3 = c === 'mp3', showWav = c === 'wav';
            bitrateSel.closest('.sw-row').style.display = showMp3 ? '' : 'none';
            chSel.closest('.sw-row').style.display = showMp3 ? '' : 'none';
            rateSel.closest('.sw-row').style.display = showWav ? '' : 'none';
            bitSel.closest('.sw-row').style.display = showWav ? '' : 'none';
        };
        refreshRecVis();
    }
    ...
    //
    // Beschriftung hängt vom Einstieg ab (@dpa ddw.md 20260803_122138 Punkt 3): in den ZWEI
    // hand-gebauten Originalen (overcord + werkbank-leer, s. lib/newEntryFlow.js
    // ORIGINAL_APPS) bleibt es "+ Neu" (neue LEERE Vorlage). In einem bereits per
    // tools/new-entry.mjs geklonten Einstieg fühlt sich derselbe Ablauf anders an — es wird
    // nicht "neu" gestartet, sondern der bereits gewachsene Stand DIESES Einstiegs wird als
    // eigenes Projekt ausgelagert. `isOutsourceMode()` ist GETEILT mit lib/newEntryFlow.js
    // (das Fenster selbst entscheidet dieselbe Frage für Titel/Texte/Kopier-Quelle) — EINE
    // Erkennung statt zweier separat gepflegter Kopien.
    const isOutsource = isOutsourceMode();
    let newEntryBtn;
    newEntryBtn = mk(
        isOutsource ? 'Auslagern' : '+ Neu',
        isOutsource
    ...
    // DATEN_INFO): "+ Neu"/"Auslagern" ist konzeptionell etwas anderes als die drei
    // Datentopf-Aktionen davor (öffnet ein eigenes Fenster statt sofort zu wirken) — das
    // sollte man beim Bedienen sehen, nicht erst nach dem Klick erraten. Die zentrale
    // "+ Neu"-Karte (NUR im Original werkbank-leer, s. dortiges index.html) erklärt denselben
    // Ablauf ausführlicher; hier reicht ein Einzeiler, weil das Fenster selbst gleich alles
    // Nötige erklärt.
    const newEntryNote = document.createElement('div'); newEntryNote.className = 'sw-note';
    i18nText(newEntryNote, isOutsource
...
const ISMS_INFO = {
    de: 'Jedes der fünf Standard-Instrumente (Tempo & MM, Scope, Debug, Rec, Meter) lässt '
        + 'sich in **seinen eigenen** Settings ausblenden – Rechtsklick auf den Instrument-'
        + 'Kopf, derselbe Knopf wie „Panel?" bei einem Control. Hier siehst du nur, welche '
        + 'davon gerade **weg** sind, und kannst sie mit einem Klick zurückholen.',
    en: 'Each of the five standard instruments (Tempo & MM, Scope, Debug, Rec, Meter) can be '
        + 'hidden from **its own** settings – right-click the instrument header, same button '
        + 'as „Panel?" on a control. This list only shows the ones currently **hidden**, one '
        + 'click brings them back.',
};

const RECFORMAT_INFO = {
    de: 'Format, in dem das **Rec**-Instrument aufnimmt — **ein** Wert für alle Aufnahmen '
        + 'dieses Einstiegs, keine Pro-Instanz-Einstellung. Bitrate/Kanäle gelten nur für MP3, '
        + 'Samplerate/Bittiefe nur für WAV.',
    en: 'Format the **Rec** instrument records in — **one** value for all recordings of this '
        + 'entry point, not a per-instance setting. Bitrate/channels apply to MP3 only, '
        + 'sample rate/bit depth to WAV only.',
};
