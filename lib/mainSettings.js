...
export function buildMainSettings(f, { state, onExport, onImport, onReset, onNewEntry, reopen, backups }) {
    ...
    let newEntryBtn;
    newEntryBtn = mk('+ Neu', 'Neues Projekt aus der leeren Werkbank vorbereiten: Name abfragen, Kollision prüfen, fertigen Terminalbefehl zeigen', () => onNewEntry(newEntryBtn));
    btnRow.append(
    ...
    // davor (legt keinen Ordner an, zeigt nur einen Befehl) — das sollte man beim Bedienen
    // sehen, nicht erst nach dem Klick erraten.
    const newEntryNote = document.createElement('div'); newEntryNote.className = 'sw-note';
    i18nText(newEntryNote, '+ Neu legt keinen Ordner selbst an — zeigt nur den fertigen Terminalbefehl zum Kopieren.');
    full(newEntryNote);
