...
export function createStepSeqEngine(state, getBeatDurMs, onTrigger) {
    ...
    let keepOnNextAnchor = false;  // '|>'-Continue: der nächste (Sprung-)Anker behält die Position, resettet NICHT auf Step 0
    ...
    return {
        ...
        transportStarted() { transportOn = true; resetPending = true; keepOnNextAnchor = false; },

        /** Transport-Continue (ddw.md 20260724_212747, '|>' = „continue ohne avv"): startet den
         * Transport, OHNE alles auf den Anfang zu ziehen — die beim Stop eingefrorene Position
         * (frozenPos) lebt weiter. `keepOnNextAnchor` unterdrückt genau EINMAL das resetPending,
         * das handleClockBeat sonst beim ersten (immer als Sprung erkannten) Beat nach Start
         * feuern würde: so ankert nur das Timing-Raster neu, die Step-Position bleibt. Gegenstück
         * zu transportStarted() (avv). */
        transportContinue() { transportOn = true; pos = frozenPos; keepOnNextAnchor = true; },
        ...
        transportStopped() { if (pos >= 0) frozenPos = pos; transportOn = false; pos = -1; nextAt = null; beatAnchorMs = null; lastBeatMs = null; resetPending = false; keepOnNextAnchor = false; },
        ...
        handleClockBeat(time) {
            ...
            lastBeatMs = time;
            // '|>'-Continue (keepOnNextAnchor): der erste Beat nach dem Start IST ein Sprung,
            // soll aber NICHT auf Step 0 zurücksetzen — nur das Raster neu ankern. Das Flag gilt
            // genau für diesen einen Anker, danach greift wieder das normale resetPending=true.
            if (jumped) { beatAnchorMs = time; nextAt = null; resetPending = !keepOnNextAnchor; keepOnNextAnchor = false; }
        ...
        tick(nowMs) {
            ...
            while (nowMs >= nextAt && guard++ < 64) {
                ...
                const v = steps[pos];
                if (v != null) onTrigger(v);   // null = off (Punkt 1, ddw.md); Skala ist ziel-abhängig, kein Clamp mehr hier
                nextAt += intervalMs();
            }
        },
    };
