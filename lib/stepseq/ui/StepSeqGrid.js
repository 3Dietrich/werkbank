...
export class StepSeqGrid {
    ...
    /** Fill-Aktion (Modus-Anschluss @dpa 20260724, s. multiSq.js wireModeButton). */
    fillNow() { this._write(fillSeq(this._steps(), this._len())); }
    /** set0-Aktion (Modus-Anschluss @dpa 20260724, s. multiSq.js wireModeButton). */
    resetNow() { this.engine.resetSeq(); }
    ...
    _build() {
        ...
        // KEIN hint()/data-hint hier (@dpa ddw.md 20260724_192304: „mouseover hier ohne
        // hilfe, sondern stattdessen … die Step nummer und sein Value anzeigen") — die
        // Hilfe-Blase würde sonst denselben Bereich beanspruchen wie die Step-Anzeige unten
        // (_showValLabel). Statt Text-Hilfe zeigt jedes Hover direkt Nummer + Wert, UNABHÄNGIG
        // vom globalen Hints-Schalter (eigenes Overlay, keine HintBubble).

        // Fill/set0 werden NICHT hier in eine gemeinsame .seq-head gehängt, sondern von
        // multiSq.js einzeln über host.mountInGroup() in die Gruppe gemountet (@dpa
        // 20260724_114012: „mach die Elemente im e-mode verschiebbar" — mountInGroup macht
        // ein Element zugleich zum eigenständigen, im e-Mode frei verschiebbaren Control,
        // genau wie beim Sq-Output-PickMenu). Steps (seqLen) ist seit @dpa 20260724_122929
        // KEIN handgebautes Zahlenfeld mehr, sondern ein GANZ NORMALER GroupHost-Knob (s.
        // lib/stepseq/defs.js KNOBS.seqLen) — der lebt komplett außerhalb von StepSeqGrid.
        // Fill/set0 je in einen eigenen <div>-Wrapper (@dpa 20260724_114012, "im e-mode
        // verschiebbar" — Bugfix beim Bauen entdeckt: css/main.css .arranging button
        // {pointer-events:none} macht rohe <button>-Elemente im e-Mode für die Maus
        // "durchsichtig", exakt wie bei jedem anderen Button-Control auch (s. GroupHost.js
        // makeButton() .btn-field-Wrapper) — mountInGroup/registerCtrlStyle müssen daher am
        // WRAPPER hängen, nicht am <button> selbst, sonst greift wireCtrlMove's mousedown nie).
        // Beide sind seit @dpa 20260724_122929 "als Button" registriert (Typ 'button', @dpa:
        // "Reset: bitte als Button" · "Fill auch") — brauchen darum je einen eigenen Außen-
        // Label wie GroupHost.makeButton()s .btn-label-Span.
        // 'ctrl-btn' (@dpa 20260724, Modus-Anschluss): teilt sich die generischen .ctrl-on/
        // .btn-fade/.ctrl-blink-Regeln (css/main.css) mit GroupHost.makeButton()s Buttons —
        // Aktivierung/Modus selbst verkabelt multiSq.js (wireModeButton, s. dort), NICHT hier.
        const fillWrap = document.createElement('div'); fillWrap.className = 'seq-ic-wrap';
        const fillLabel = document.createElement('span'); fillLabel.className = 'btn-label';
        const fill = document.createElement('button'); fill.type = 'button'; fill.className = 'pb-btn seq-ic ctrl-btn'; fill.textContent = 'Fill';
        hint(fill, 'Fill: sichtbares Muster über den unsichtbaren Rest wiederholen');
        fillWrap.append(fillLabel, fill);
        const set0Wrap = document.createElement('div'); set0Wrap.className = 'seq-ic-wrap';
        const set0Label = document.createElement('span'); set0Label.className = 'btn-label';
        const s0 = document.createElement('button'); s0.type = 'button'; s0.className = 'pb-btn seq-ic ctrl-btn'; s0.textContent = '⟲';
        hint(s0, 'set0: der nächste Trigger startet wieder bei Step 1');
        set0Wrap.append(set0Label, s0);
        // Nach außen exponiert, damit multiSq.js sie einzeln mounten/registrieren kann
        // (eigene Optik-Settings UND eigene Verschiebbarkeit je Knopf, wie jeder andere
        // Control). fillBtn/set0Btn bleiben die ECHTEN Knöpfe (Optik/Text wird weiter direkt
        // auf sie angewandt), fillWrap/set0Wrap sind die beweglichen/rechtsklickbaren Hüllen.
        this.fillBtn = fill; this.set0Btn = s0; this.fillLabel = fillLabel; this.set0Label = set0Label;
        this.fillWrap = fillWrap; this.set0Wrap = set0Wrap;
        ...
        // Live-Wertanzeige beim Ziehen (@dpa ddw.md 20260724_153349: "beim draggen bitte den
        // aktuellen drag Wert anzeigen ... in beiden An/Aus modi") — schwebt über dem Step,
        // den man gerade zieht. box braucht dafür position:relative als Bezugsrahmen.
        box.style.position = 'relative';
        const dragVal = document.createElement('div'); dragVal.className = 'seq-drag-val'; dragVal.hidden = true;
        this._dragVal = dragVal;

        box.append(cv, dragVal);
        this.refresh();
        return box;
    }
    ...
    /** Anzeigeformat für den Drag-Wert: ganzzahlig, wenn Stepsize/Skala es hergeben, sonst 2 Nachkomma. */
    _fmtVal(v) {
        const step = this._stepSize();
        if (step > 0 && Number.isInteger(step)) return String(Math.round(v));
        return (Math.round(v * 100) / 100).toString();
    }
    /** Schwebe-Label Step-Nummer + Wert — sowohl beim Ziehen als auch beim reinen Hover
     *  (@dpa ddw.md 20260724_192304). v=null (nur im An/Aus-Modus möglich) zeigt „–". */
    _showValLabel(p, i, v) {
        this._dragVal.textContent = '#' + (i + 1) + '  ' + (v == null ? '–' : this._fmtVal(v));
        this._dragVal.hidden = false;
        const w = this._w();
        this._dragVal.style.left = Math.max(2, Math.min(w - 2, p.x)) + 'px';
        this._dragVal.style.top = Math.max(0, p.y - 4) + 'px';
    }
    _hideDragVal() { this._dragVal.hidden = true; }
    ...
    _wire(cv) {
        ...
        const setAt = (p) => {
            ...
            this._lastH[i] = v;
            this._write(arr);
            this._showValLabel(p, i, v);   // @dpa ddw.md 20260724_153349: Wert live beim Ziehen zeigen
        };
        const onMove = (e) => {
            if (!start) return;
            const p = at(e);
        ...
        // Reines Hover (kein Klick/Ziehen): Step-Nummer + aktueller Wert statt Hilfe-Blase
        // (@dpa ddw.md 20260724_192304). Läuft während `start` NICHT mit — dann übernimmt
        // onMove/setAt oben dieselbe Anzeige mit dem live gezogenen Wert.
        cv.addEventListener('mousemove', (e) => {
            if (start) return;
            const p = at(e);
            const i = this._stepAtX(p.x);
            this._showValLabel(p, i, this._steps()[i]);
        });
        cv.addEventListener('mouseleave', () => { if (!start) this._hideDragVal(); });
    }
    ...
    _draw() {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (raw != null) {
                ...
                let bh = Math.max(norm > 0 ? 1 : 0, norm * (H - 2));   // ein winziger Wert bleibt sichtbar
                // @dpa ddw.md 20260724_153349: im Werte-Modus (An/Aus AUS) gibt es kein echtes
                // "Off" mehr — ein Step auf Minimum sähe sonst genauso leer aus wie ein Step,
                // der im An/Aus-Modus ausgeschaltet ist. Ein schmaler Boden-Streifen zeigt:
                // "Minimum, nicht Off." — 3px, nicht 2: die unterste Reihe frisst am Ende
                // sowieso der Rahmen-Stroke (strokeRect unten), sonst bliebe nur 1px sichtbar.
                if (!this._offMode() && bh < 3) bh = 3;
