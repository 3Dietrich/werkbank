...
export function stepSeqDefs() {
    return {
        // Wert-Defaults eines NEU per [+] gebauten Sequenzers (@dpa 20260725: „den ersten
        // Sequenzer aus der json als default ('+') nehmen" — Wahl „Optik + Speed/Länge"):
        // Speed/Länge von Sequenzer 0 (Teiler 8, Step-Zahl 13) werden Default, das Muster
        // (seqSteps) und das Output-Ziel bleiben bewusst leer — jeder neue Sq wird eigen belegt.
        DEFAULTS: {
            seqEnabled: false,   // AN = Basisclock ist scharf, läuft aber nur bei laufendem Transport
            seqLen: 13,          // sichtbare/aktive Steps (1..SEQ_MAX) — Seq-0-Vorbild (@dpa 20260725)
            seqSteps: makeSeqSteps('first'),  // KEIN festes Muster als Default (nur Step 0 an, Rest off)
            ...
            // BaseFreq-gekoppelter Insel-Puls mehr. seqDiv=8 (statt 1) ist die Seq-0-Vorlage.
            seqMult: 1,
            seqDiv: 8,           // Seq-0-Vorbild (@dpa 20260725), vorher 1
            ...
            // GROUPS/SELECTS, s. dort „warum kein <select>"). Leer = „— kein Ziel —" (@dpa
            // 20260725: neuer Sq soll ohne Ziel starten, nicht auf ein zufälliges trig zeigen).
            seqOutput: '',
        },

        // Default-ANSICHT des Step-Grids eines NEU gebauten Sq (@dpa 20260725, s. DEFAULTS oben):
        // die grüne, breite Optik von Sequenzer 0. multiSq.js buildSq() schreibt das als
        // ctrlStyle 'u:seqGrid_i', WO NOCH KEINE gespeicherte Ansicht steht (Recall/Snapshot
        // gewinnt). Ohne dies bekäme ein neuer Sq den grauen StepSeqGrid.DEFAULT_STYLE.
        // BEWUSST OHNE seqMin/seqMax/seqStep (@dpa 20260727: „nie auf einen Default-Max
        // limitieren, unsichtbar für den User") — das hätte JEDES neu gewählte Ziel (auch
        // eins mit viel größerem Eigenbereich, z.B. Höhen-Dämpf 0..200) unsichtbar auf
        // 1..15 gedeckelt. Skala folgt jetzt IMMER der Ziel-Range (StepSeqGrid._target),
        // bis der User über die Data-Sektion (Element-Settings) bewusst überschreibt.
        GRID_STYLE: { bg: '#244f4b', fg: '#82ba90', boxSize: 315, boxH: 50, seqOff: false },
        ...
        KNOBS: {
            ...
            // Label 'Steps' → 'Step-Zahl' (@dpa ddw.md 20260724_153349): hieß bisher genau wie
            // das Step-Grid selbst (u:seqGrid, registerCtrlStyle-Label 'Steps') — zwei
            // gleichnamige, aber verschiedene Dinge (der eine ein normaler Combo-Knob, der
            // andere bewusst NICHT im Combo, s. multiSq.js comboExcludeKeys) führten zu Verwechslung.
            seqLen: { label: 'Step-Zahl', min: 1, max: 64, step: 1, curve: 'linear', unit: '', decimals: 0,
