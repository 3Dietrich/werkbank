...
export function createSqManager({ host, state, defs, tpl, getBeatDurMs, routing, createEngine, makeGrid }) {
    ...
    return {
         ...
         * Nötig nach einem Snapshot-Recall (@dpa 20260725): der ISM-Snapshot setzt die
         * Wert-Keys (seqLen_i …) UND sqCount zurück in den State, baut die GroupHost-Gruppen
         * aber NICHT — buildSq() läuft sonst nur in init(). Ohne Reconcile stünden nach dem
         * Recall die alten (evtl. gelöschten) Sqs weiterhin nicht da, obwohl sqCount sie
         * verlangt. buildSq() setzt Defaults nur, wo der State leer ist → die schon vom
         * Recall gesetzten Wert-Keys gewinnen (die Sqs kommen „ready to run" zurück).
         */
        reconcile() {
