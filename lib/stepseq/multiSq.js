...
/** Optik für Fill-/set0-Knopf. Beide sind seit @dpa 20260724_122929 Typ 'button' (@dpa:
 *  "Reset: bitte als Button" · "Fill auch") — bekommen darum einen Außen-Label wie ein
 *  normaler GroupHost-Button, wenn wrap/labelEl mitgegeben werden. */
function applyMiniBtnStyle(btn, s, defText, wrap, labelEl) {
...
export function createSqManager({ host, state, defs, tpl, getBeatDurMs, routing, createEngine, makeGrid }) {
    ...
    function buildSq(i) {
        ...
        host.registerCtrlStyle('u:seqFill' + sfx, 'button', grid.fillWrap, (s) => applyMiniBtnStyle(grid.fillBtn, s, 'Fill', grid.fillWrap, grid.fillLabel), 'Fill');
        host.mountInGroup(sqName(i), grid.set0Wrap, 'u:seqSet0' + sfx);
        host.registerCtrlStyle('u:seqSet0' + sfx, 'button', grid.set0Wrap, (s) => applyMiniBtnStyle(grid.set0Btn, s, '⟲', grid.set0Wrap, grid.set0Label), 'Reset');
