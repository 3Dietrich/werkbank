...
export function createSqManager({ host, state, defs, tpl, getBeatDurMs, routing, createEngine, makeGrid }) {
    ...
    function buildSq(i) {
        ...
        // 3) Gruppe bauen (Toggles/Selects/Knobs indiziert)
        host.addGroup({
            name: sqName(i),
        ...
        // 4) Engine + Grid an den scoped State hängen; Grid als u:-Control in die Gruppe.
        const st = scoped(state, sfx);
        const eng = createEngine(st, getBeatDurMs, (h) => {
            if (state.get('seqOutput' + sfx) !== 'AmpEnv+OSZ') return;   // andere Ziele: Sonnet (Punkt 1)
            routing.emit({ module: 'stepseq', port: 'amp' }, h);
        });
    ...
    return {
        ...
        removeSq() {
            ...
            engines.splice(i, 1); grids.splice(i, 1);
