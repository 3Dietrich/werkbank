...
export function createSqManager({ host, state, defs, tpl, getBeatDurMs, routing, createEngine, makeGrid }) {
    ...
    function buildSq(i) {
        ...
        host.addGroup({
            ...
            // gemeint ist das Step-GRID (u:seqGrid, seine Data-Sektion: seqMin/seqMax/seqStep/
            // seqOff + bg/fg), NICHT der Step-ZAHL-Knob (seqLen, s. defs.js Label 'Step-Zahl').
            // Klarstellung @dpa 20260724_153349, nachdem die erste Fassung hier fälschlich
            // 'seqLen' statt 'seqGrid' ausgeschlossen hatte: der Knob soll wie jeder andere
            // ganz normal in den geteilten Sequenzer-Combo einfließen, nur die Grid-Ansicht
            // nicht. Snapshot bleibt unberührt (extraSoundKeys/VALUE_KEYS oben, nicht hier).
            comboExcludeKeys: ['seqGrid'],
        });
