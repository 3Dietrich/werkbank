...
export function createSqManager({ host, state, defs, tpl, getBeatDurMs, routing, createEngine, makeGrid }) {
    ...
    function buildSq(i) {
        ...
        const eng = createEngine(st, getBeatDurMs, (v) => {
            ...
            // lastIndexOf statt indexOf (Bugfix ddw.md 20260726, @dpa: „Ziel/Moduleingang
            // von z.B. Seq funktioniert noch nicht"): Modul-IDs wie 'polysynth.env_0'
            // (Multi-ADSR-Instanzen) enthalten selbst einen Punkt — der ERSTE Punkt hätte
            // 'polysynth' + 'env_0.gate' ergeben (Ziel existiert nie), statt korrekt
            // 'polysynth.env_0' + 'gate'.
    ...
    function restoreViews(v) {
        ...
        // Optik auch auf die LEBENDEN Controls malen (@dpa 20260725: „Sequenzer-Höhe erst
        // beim Anfassen von max aktualisiert") — GroupHost abonniert ctrlStyles/knobMeta
        // NICHT (die setzt sonst nur der eigene Editor), darum muss der Recall selbst
        // nachziehen. ctrlPos/groupPos/… laufen über die bestehende subscribe-Kette.
        // Nur die Sq-eigenen IDs anfassen (…_<i>), fremde ISM-Controls bleiben unberührt.
        host.reapplyCtrlStyles(Object.keys(v.ctrlStyles || {}));
        host.reapplyKnobMeta(Object.keys(v.knobMeta || {}));
    }
    ...
    return {
        ...
        /** @param {object} [values]  die im Snapshot gespeicherten Sound-Werte (roh, VOR dem
         *  Aufruf schon in den State geschrieben) — nur als Fallback genutzt, wenn `extra`
         *  kein sqCount trägt (@dpa dd.md 20260802: "Space > Play"/"Demo 2" luden nicht mehr
         *  richtig, "es sind zu viele Sequenzen"). Altbestand-Snapshots von VOR dem sqCount/
         *  extra-Umbau (932ef4f) haben gar kein `extra` — ohne Fallback bliebe reconcile() beim
         *  AKTUELLEN (evtl. inzwischen gewachsenen) Sq-Stand stehen, überzählige Sqs behielten
         *  ihre Werte von VOR dem Recall statt zurückgesetzt/abgebaut zu werden. Die Anzahl lässt
         *  sich aber aus dem höchsten in `values` vorkommenden Sq-Index rekonstruieren. */
        recallSnapshot(extra, values) {
            let count = extra && extra.sqCount != null ? extra.sqCount : null;
            if (count == null && values) {
                let maxIdx = -1;
                for (const k of Object.keys(values)) {
                    const m = /^(.+)_(\d+)$/.exec(k);
                    if (m && VALUE_KEYS.includes(m[1])) maxIdx = Math.max(maxIdx, +m[2]);
                }
                if (maxIdx >= 0) count = maxIdx + 1;
            }
            if (count != null) state.set('sqCount', count);
