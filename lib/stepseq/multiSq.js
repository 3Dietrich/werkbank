...
export function createSqManager({ host, state, defs, tpl, getBeatDurMs, routing, createEngine, makeGrid }) {
    ...
    function buildSq(i) {
        ...
        // Fill/set0/Steps-Feld (@dpa 20260724: "Speicher wie bei Control/Knob-Settings", dann
        ...
        // andere Button-Control auch), der Wrapper dahinter bleibt greifbar.
        host.mountInGroup(sqName(i), grid.fillWrap, 'u:seqFill' + sfx);
        host.registerCtrlStyle('u:seqFill' + sfx, 'opener', grid.fillWrap, (s) => applyMiniBtnStyle(grid.fillBtn, s, 'Fill'), 'Fill');
        host.mountInGroup(sqName(i), grid.set0Wrap, 'u:seqSet0' + sfx);
        host.registerCtrlStyle('u:seqSet0' + sfx, 'opener', grid.set0Wrap, (s) => applyMiniBtnStyle(grid.set0Btn, s, '⟲'), 'Reset');
        host.mountInGroup(sqName(i), grid.stepsField, 'u:seqSteps' + sfx);
