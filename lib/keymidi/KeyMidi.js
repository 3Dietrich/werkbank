...
export class KeyMidi {
    ...
    /* ── Radio-Selektion im Overlay (Punkt D): genau EIN Control markiert ─────────────── */
    _selectCtrl(el) {
        if (this._selectedEl === el) return;
        this._clearSelect();
        this._selectedEl = el;
        if (el) el.classList.add('km-selected');
    }
    _clearSelect() {
        if (this._selectedEl) this._selectedEl.classList.remove('km-selected');
    ...
    _listenKey(t, keyEl) {
        ...
        const grab = (e) => {
            e.preventDefault(); e.stopPropagation();
            if (NO_LEARN.has(e.key)) return done();       // ESC/Enter/Tab nicht lernen → Abbruch (Punkt D)
