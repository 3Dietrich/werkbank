 ...
 *
 * Zero-Attack-Trim (@dpa 20260805: „der max. audio wert bei Att=0 liegt bei amp≈1.2 …
 * bitte dividieren? sonst nicht"): der Limiter-Algorithmus (egal ob nativ oder Worklet)
 * begrenzt mit RATIO=20 — endlich, kein echtes Brickwall. Bei Attack=0 und extrem hohen
 * Eingangspeaks (viele Stimmen/Obertöne kurz phasengleich) lässt das den Ausgang trotzdem
 * bis ~1.2 durch, GENAU in der Kombi ohne WaveShaper-Sicherheitsnetz danach. @dpa hat NUR
 * diese eine gehörte Kombi (Lim=on, Att=0, WS=off) korrigiert haben wollen, „sonst nicht" —
 * andere Attack-Werte sind ein eigener, noch nicht gehörter Hördurchgang (@dpa-Regel „ein
 * Change pro Hördurchgang"). `zeroAttackTrim` ist deshalb ein eigener, permanent in der
 * Kette hängender GainNode NACH dem Limiter (s. rebuildChain), dessen Gain NUR in exakt
 * dieser Kombi auf 1/1.2 gesetzt wird (updateZeroAttackTrim()) — in jeder anderen Kombi
 * bleibt er bei 1.0 (unhörbar, reine Durchleitung).
 */
...
function rebuildChain() {
    ...
    [volumeGain, limiter, zeroAttackTrim, waveshaperPreGain, waveshaper].forEach((node) => {
        if (!node) return;
        try { node.disconnect(); } catch { /* schon getrennt */ }
    });
    let node = volumeGain;
    if (limiterOn && limiter) {
        node.connect(limiter); node = limiter;
        // zeroAttackTrim sitzt IMMER direkt nach dem Limiter (nie ohne ihn) — s. Kopfkommentar
        // „Zero-Attack-Trim". Gain steht bei 1.0, außer in der einen gehörten Sonderkombi.
        if (zeroAttackTrim) { node.connect(zeroAttackTrim); node = zeroAttackTrim; }
    }
...
/** Setzt den Gain von `zeroAttackTrim` — 1/1.2 NUR in exakt der einen von @dpa gehörten
 *  Sonderkombi (Lim=on, Att=0, WS=off), sonst 1.0 (s. Kopfkommentar „Zero-Attack-Trim"). */
function updateZeroAttackTrim() {
    if (!zeroAttackTrim) return;
    const special = limiterOn && curAttackMs === 0 && !waveshaperOn;
    zeroAttackTrim.gain.value = special ? 1 / 1.2 : 1;
}
...
export function ensureAudio() {
    ...
    zeroAttackTrim = audio.createGain();   // s. Kopfkommentar „Zero-Attack-Trim"
    updateZeroAttackTrim();
...
export function getZeroAttackTrim() { return zeroAttackTrim; }   // Debug/Test
...
export function setLimiterOn(on) { limiterOn = !!on; rebuildChain(); updateZeroAttackTrim(); }
...
export function setWaveshaperOn(on) { waveshaperOn = !!on; rebuildChain(); updateZeroAttackTrim(); }
