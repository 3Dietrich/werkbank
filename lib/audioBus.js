...
/**
 * Baut die Soft-Clip-Kurve für den WaveShaper: klassischer tanh-Sättiger, transparent nahe
 * 0, sanft Richtung ±1 abgeflacht (kein hartes Brickwall wie der Limiter — ein zusätzliches,
 * weiches Netz DAHINTER). `k` ~3 (@dpa-Vorgabe „Standard-Soft-Clip") bestimmt die Schärfe,
 * `points` die Auflösung der Lookup-Tabelle (2048 = glatt genug, kein hörbares Treppen).
 */
function buildSoftClipCurve(k = 3, points = 2048) {
    const curve = new Float32Array(points);
    const norm = Math.tanh(k);   // normiert curve(±1) auf genau ±1 (kein Pegelsprung an der Nahtstelle zu destination)
...
export function ensureAudio() {
    ...
    waveshaper.curve = buildSoftClipCurve(3, 2048);
