...
import { hint, t as i18nT, text as i18nText } from './i18n.js';
...
export class PickMenu {
  ...
  _fill() {
    ...
    if (!items.length) {
      ...
      i18nText(e, 'noch nichts gespeichert');
      list.appendChild(e);
      return;
    }
    let curRow = null;
    items.forEach((it, i) => {
      ...
      // Übersetzung nur des STATISCHEN Teils (i18nT), it.name bleibt unangetastet (selbst
      // vergebener Name) — hint() bekäme sonst einen aus DE+EN gemischten String als
      // „Original" und fände dafür nie einen EN-Eintrag (@dpa ddw.md 20260724_183901,
      // „fehlende Übersetzungen … die Hilfen im Speicher sind noch deutsch").
      if (this._cfg.onUpdate) act('edit', `„${it.name}" ${i18nT('mit dem aktuellen Zustand überschreiben')}`, this._cfg.onUpdate, 'save');
      if (this._cfg.onRename) act('tag', `„${it.name}" ${i18nT('umbenennen')}`, (idx, item) => this._rename(idx, item), 'ren');
      if (this._cfg.onDelete) act('trash', `„${it.name}" ${i18nT('löschen')}`, this._cfg.onDelete, 'del');
  ...
  _rename(i, it) {
    const nm = prompt(i18nT('Neuer Name für') + ' „' + it.name + '"?', it.name);
    if (nm === null || nm.trim() === it.name) return;
    const err = this._cfg.onRename(i, it, nm.trim());
    if (err) alert(i18nT(err));
  }
