...
export class SignalScope {
    ...
    constructor(opts = {}) {
        ...
        console.log('[SignalScope] CONSTRUCTOR — defaults:', {width: DEFAULTS.width, height: DEFAULTS.height, bufferMs: DEFAULTS.bufferMs});
    ...
    applyStyle(s = {}) {
        console.log('[SignalScope.applyStyle] CALLED with:', s);
        console.log('[SignalScope.applyStyle] this.style BEFORE merge:', this.style);
        this.style = { ...this.style, ...s };
        console.log('[SignalScope.applyStyle] this.style AFTER merge:', this.style);
        const st = this.style;
        const w = Math.max(24, st.width | 0), h = Math.max(12, st.height | 0);
        const dpr = window.devicePixelRatio || 1;
        console.log('[SignalScope.applyStyle] width/height:', { input_width: st.width, input_height: st.height, clamped_w: w, clamped_h: h, dpr });
        this._canvas.width = w * dpr;
        this._canvas.height = h * dpr;
        this._canvas.style.width = w + 'px';
        this._canvas.style.height = h + 'px';
        console.log('[SignalScope.applyStyle] canvas.width/height:', { canvas_width: this._canvas.width, canvas_height: this._canvas.height, style_width: this._canvas.style.width, style_height: this._canvas.style.height });
