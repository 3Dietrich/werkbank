...
export class SignalScope {
    ...
    _dragTriggerFromEvent(e) {
        ...
        this.setTriggerPos(x / Math.max(1, rect.width));
    }
    ...
    _setupTap() {
        ...
        proc.onaudioprocess = (e) => {
            if (ring.anchorTime == null) ring.anchorTime = e.playbackTime;
     ...
     *  Pfad nicht an). */
    _syncTap() {
        const want = this._syncOn && this._hasNode;
    ...
    _hoverReadout(e) {
        ...
        if (kind === 'ring' && data.length) {
            const i = Math.round(x * (data.length - 1));
            v = data[i];
    ...
    tick() {
        ...
        if (st.showCurve && special && special.kind === 'ring' && special.data.length > 1) {
            const n = special.data.length;
            c.strokeStyle = st.curveColor;
            c.lineWidth = 1.5;
            c.beginPath();
            for (let i = 0; i < n; i++) {
                const x = (i / (n - 1)) * curveW;
                const py = y(special.data[i]);
                if (i === 0) c.moveTo(x, py); else c.lineTo(x, py);
            }
            c.stroke();
            this._last = special.data[n - 1];
            if (special.triggerT != null) this._drawTriggerLine(c, special, curveW, h);
        } else if (st.showCurve && special && special.kind === 'points' && special.data.length > 1) {
            const { data, t0, t1 } = special;
            c.strokeStyle = st.curveColor;
            c.lineWidth = 1.5;
            c.beginPath();
            let started = false;
            for (const p of data) {
                const x = ((p.t - t0) / Math.max(1e-9, t1 - t0)) * curveW;
                const py = y(p.v);
                if (!started) { c.moveTo(x, py); started = true; } else c.lineTo(x, py);
            }
            c.stroke();
            this._last = data[data.length - 1].v;
    ...
    /** Dünne Trigger-Linie an der Beat-Position im Sync-Fenster. */
    _drawTriggerLine(c, special, curveW, h) {
        const x = ((special.triggerT - special.t0) / Math.max(1e-9, special.t1 - special.t0)) * curveW;
        c.save();
        c.globalAlpha = 0.5;
