...
export function factoryHint(id, lang = 'de') {
    // Sq-Klone hängen ihre laufende Nummer an (t:seqEnabled_0, k:seqMult_1, …, s. multiSq.js)
    // — die Kennung hier ist aber immer die BASIS-Kennung, ein Sq-Klon braucht keinen eigenen
    // Hilfetext-Eintrag (@dpa ddw.md 20260724_153349: „die Hilfstexte auch!").
    const h = HINTS[id] || HINTS[id.replace(/_\d+$/, '')];
    if (!h) return '';
