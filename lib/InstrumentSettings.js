 ...
 * `setCollapsed(false)` (Aufklappen) lädt die Seite neu, statt live nachzumessen (@dpa ddw.md
 * 20260805) — ein ISM, das seit dem letzten Laden eingeklappt war, wurde von GroupHost VOR
 * diesem Konstruktor unsichtbar gemessen (Panel-Größe fror sich falsch ein); ein Reload baut
 * es im Zielzustand von Grund auf neu. Einklappen bleibt sofort/live.
 */
...
export function mountInstrumentSettings(sectionEl, state, opts = {}) {
    ...
    const setCollapsed = (v) => {
        ...
        // 20260805, „zeigt danach keinen Inhalt mehr nach dem ausklappen"): mountGroups()
        ...
        // Mehrere gezielte Nachmess-Patches dagegen blieben lückenhaft. Pragmatischer Ersatz:
        // beim AUFKLAPPEN neu laden — das ISM baut sich dann von Grund auf im sichtbaren
        // Zielzustand auf, exakt wie jedes nie eingeklappte ISM auch (Zustand ist über
        // MiniState.set() synchron persistiert, bevor reload() greift). Einklappen bleibt
        // sofort/live — das funktionierte schon immer korrekt (s. GroupHost.js
        // setGroupCollapsed, dieselbe Begründung dort für Gruppen).
