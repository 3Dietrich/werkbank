 ...
 * @param {() => object} [opts.snapExtra]  ISM-weite Zusatz-Keys für den Snapshot, die zu keiner
 *        Gruppe gehören (Stepseq: `sqCount`). Werden beim Speichern über host.allSoundValues()
 *        gemischt und beim Recall mit zurückgesetzt.
 * @param {() => void} [opts.onSnapRecalled]  Hook NACH dem Zurücksetzen aller Snapshot-Werte —
 *        fürs Angleichen der ISM-Struktur an den geladenen State (Stepseq: multiSq.reconcile()).
...
export function mountInstrumentSettings(sectionEl, state, opts = {}) {
    ...
    // opts.snapExtra (@dpa 20260725): ISM-weite Zusatz-Keys, die zu KEINER Gruppe gehören und
    // die host.allSoundValues() (gruppen-basiert) darum nicht erfasst — beim Stepseq-ISM z.B.
    // `sqCount` (wie viele Sequenzer-Gruppen es gibt). Ohne diese käme nach dem Recall die
    // Sq-ANZAHL nicht zurück. opts.onSnapRecalled läuft NACH dem Zurücksetzen aller Werte, damit
    // das ISM seine Struktur an den frisch geladenen State angleichen kann (Stepseq: neu/zu viel
    // gebaute Sq-Gruppen reconcilen, s. multiSq.reconcile()).
    const listIsmSnaps = () => state.get('ismSnaps') || [];
    const snapValues = () => ({ ...host.allSoundValues(), ...(opts.snapExtra ? opts.snapExtra() : {}) });
    function saveIsmSnap(entryName) {
        const list = listIsmSnaps().slice();
        const entry = { name: entryName, ts: Date.now(), values: snapValues() };
    ...
    function updateIsmSnap(index) {
        ...
        list[index] = { ...list[index], ts: Date.now(), values: snapValues() };
        state.set('ismSnaps', list);
        return true;
    }
    ...
    function recallIsmSnap(index) {
        ...
        // NACH allen Werten: das ISM seine Struktur an den geladenen State angleichen lassen
        // (Stepseq baut hier die im Snapshot verlangten Sq-Gruppen neu/ab, s. multiSq.reconcile).
        if (opts.onSnapRecalled) opts.onSnapRecalled();
        return true;
    }
