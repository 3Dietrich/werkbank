 ...
 * `setCollapsed(false)` (Aufklappen) ruft zusätzlich `opts.host.resyncSizes()` (@dpa ddw.md
 * 20260805, „mach das, was e-mode beim Einschalten macht — ohne die Ansicht ändern zu
 * müssen") — ein ISM, das seit dem letzten Laden eingeklappt war, wurde von GroupHost VOR
 * diesem Konstruktor unsichtbar gemessen (Panel-/Free-Canvas-Größe fror sich falsch ein);
 * resyncSizes() ist derselbe freezeGroup()-Lauf, den Anordnen-Modus beim Einschalten fährt,
 * nur ohne dessen restliche Optik (Rahmen/Hint/Knob-Sperre). Einklappen bleibt sofort/live.
 */
...
export function mountInstrumentSettings(sectionEl, state, opts = {}) {
    ...
    const setCollapsed = (v) => {
        state.set('instrCollapsed', v || undefined);
        applyCollapsed();   // Sektion sichtbar/unsichtbar schalten — MUSS vor resyncSizes() stehen
        // Aufklappen eines ISM, das SEIT DEM LETZTEN LADEN eingeklappt war (@dpa ddw.md
        // 20260805, „zeigt danach keinen Inhalt mehr nach dem ausklappen", dann: „mach das, was
        // e-mode beim Einschalten macht — ohne die Ansicht ändern zu müssen"): mountGroups()
        // baut/misst sein GroupHost-Panel VOR diesem Konstruktor — startete die Sektion
        // eingeklappt, maß GroupHost sie unsichtbar und fror die Panel-Größe falsch ein.
        // host.resyncSizes() (mountGroups()-Rückgabe, falls ein ISM eins übergeben hat) ist
        // GENAU der freezeGroup()-Lauf, den e-Mode beim Einschalten fährt — hier ohne den Rest
        // von Arrangieren (kein Rahmen/Hint/Knob-Sperre), leichter als ein voller Reload.
        // Einklappen bleibt sofort/live — das funktionierte schon immer korrekt.
        if (!v && opts.host && opts.host.resyncSizes) opts.host.resyncSizes();
    };
