...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    function makeButton(key) {
        ...
        let isOn = false, curBg = '', curBgOn = '', textOn = cfg.label, textOff = cfg.label, outerLabel = '';
        const paint = () => {
            btn.textContent = '';
            ...
            label.style.display = outerLabel ? '' : 'none';
        };
        paint();
        btn.refresh = paint;
        ...
        const trigger = () => { if (arranging) return; isOn = !isOn; onClick(key); paint(); };
        btn.addEventListener('click', trigger);
        keyMidi.register('b:' + key, wrap, cfg.label, trigger);
        ctrlOnSetters.set('b:' + key, (on) => { isOn = !!on; paint(); });
