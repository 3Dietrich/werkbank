...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    function makeButton(key) {
        ...
        const fire = () => { if (!arranging) onClick(key); };
        const flashTrigger = () => {
    ...
    function makeSpecial(cfg) {
        ...
        function close() { if (pop) { pop.remove(); pop = null; document.removeEventListener('mousedown', outside, true); paintOpener(); } }
        paintOpener();
    ...
    function openGroupSettings(name, anchor) {
        ...
        const mkColor = (labelTxt, prop, defHex) => {
            ...
            row(labelTxt, col, a, clr);
        };
        mkColor('BG', 'bg', '#1c2027');
        mkColor('Head', 'headColor', '#8a93a3');
