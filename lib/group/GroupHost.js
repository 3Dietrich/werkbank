...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    function setGroupCollapsed(name, on) {
        setGroupStyle(name, { collapsed: on });
        // Aufklappen einer Gruppe, die SEIT DEM LETZTEN LADEN eingeklappt war (@dpa ddw.md
        // 20260805, „auch die Gruppen-Klappe ist noch nicht überall gefixt", dann: „mach das,
        // was e-mode beim Einschalten macht — ohne die Ansicht ändern zu müssen"): Free-Canvas-
        // Rahmenmaß (sizeFreeGroup) und Panel-Größe (sizePanel) werden nur EINMAL beim Bau
        // korrekt gemessen, solange alles da schon sichtbar ist. Statt eines vollen Reloads
        // reicht der leichte e-Mode-Resync — resyncSizes() ist derselbe freezeGroup()-Lauf, den
        // setArranging(true) fährt, nur ohne Auswahlrahmen/Hint-Banner/Knob-Sperre. Einklappen
        // bleibt sofort/live — das funktionierte schon immer korrekt.
        if (!on) resyncSizes();
    }
    ...
    function makeOffPanelRow(id) {
        ...
        // data-ctrl auch hier setzen (@dpa 20260805: „Controls sollten auch in Gruppen-
        // Settings ihre MO-helphints anzeigen") – HintBubble sucht global nach
        // `[data-ctrl]`; ohne dieses Attribut blieb die Off-Panel-Zeile stumm, obwohl
        // dasselbe Control auf dem echten Panel längst seinen Hover-Hint zeigt.
        row.dataset.ctrl = id;
    ...
    // resyncSizes() (@dpa ddw.md 20260805, „was e-mode beim Einschalten macht — ohne die
    // Ansicht ändern zu müssen"): der freezeGroup()-Lauf über ALLE Gruppen war schon immer der
    // bekannte manuelle Fix „kurz in e-Mode und wieder raus" (misst Free-Canvas-Rahmenmaße neu,
    // die beim letzten Bau evtl. unsichtbar/falsch gemessen wurden). Hier als eigener,
    // eigenständig aufrufbarer Baustein herausgezogen — setArranging(true) nutzt ihn
    // weiterhin 1:1, zusätzlich ruft ihn setGroupCollapsed()/InstrumentSettings.js beim
    // Aufklappen auf, OHNE den Rest von Arrangieren-Modus (kein Auswahlrahmen, kein
    // Hint-Banner, keine Knob-Sperre) — leichter als ein voller Seiten-Reload.
    function resyncSizes() { for (const name of groupEls.keys()) freezeGroup(name); sizePanel(); }
    ...
    return {
        ...
        // Leichter e-Mode-Resync ohne Moduswechsel (@dpa ddw.md 20260805, s. Kommentar an der
        // Definition) — für Aufrufer, die NUR eine kaputte Größe reparieren wollen (z.B.
        // InstrumentSettings.js beim ISM-Aufklappen), nicht den vollen Reload/Refresh.
        resyncSizes,
    };
}
