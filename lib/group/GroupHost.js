...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    function makeButton(key) {
        ...
        const flashTrigger = () => {
            ...
            requestAnimationFrame(() => { btn.classList.add('btn-fade'); isOn = false; paint(); }); // mit Transition: fadet aus
        };
    ...
    panel.addEventListener('mousedown', (e) => {
        ...
        if (e.shiftKey && ctrl) {
            e.preventDefault(); e.stopPropagation();
            if (selected.has(ctrl)) removeSelected(ctrl); else addSelected(ctrl);
            return;
        }
    ...
    function applyGroupStyles(data) {
        ...
        for (const [name, e] of groupEls) {
            ...
            // Größe % (@dpa 20260720, Punkt G): skaliert die ganze Gruppe. `zoom` (statt transform)
            // reflowt die Nachbarn mit, statt nur optisch zu skalieren. 100/leer = neutral.
            e.g.style.zoom = (st.scale && st.scale !== 100) ? (st.scale / 100) : '';
    ...
    function sizePanel() {
        ...
        _sizeRaf = requestAnimationFrame(() => {
            let maxB = 0, maxR = 0;
            for (const { g } of groupEls.values()) { maxB = Math.max(maxB, g.offsetTop + g.offsetHeight); maxR = Math.max(maxR, g.offsetLeft + g.offsetWidth); }
            panel.style.height = maxB + 'px'; panel.style.width = maxR + 'px';
        });
    }
