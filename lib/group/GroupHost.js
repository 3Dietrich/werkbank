...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    // 'layerTop' (@dpa ddw.md Z.875, ElementSettings.js „User-Lösung"): cross-cutting, gilt
    // für ALLE Control-Sorten gleich — deshalb HIER als Wrapper um jedes typ-eigene applyStyle
    // gesetzt statt in jeder makeX()-Fabrik einzeln. Hebt den Wrapper per z-index vor
    // überlappende Geschwister (das freie Panel setzt Controls schon auf position:absolute,
    // s. css/main.css .free-canvas > [data-ctrl]; Hover hebt dort auf z-index 5, „oben" muss
    // darüber liegen, damit es auch OHNE Hover sichtbar bleibt).
    function registerCtrlStyle(id, type, el, applyStyleRaw, defLabel, extra = {}) {
        const applyStyle = (s) => { el.style.zIndex = s && s.layerTop ? '6' : ''; applyStyleRaw(s); };
