...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    function applyOffPanel() {
        ...
        for (const id of ids) { const el = ctrlElById(id); if (el) el.classList.toggle('ctrl-offpanel', !!flags[id]); }
    }
    ...
    function makeOffPanelRow(id) {
        ...
        if (type === 'k') {
            ...
            // Text-Zeile, sie bleiben rein visuell dem Panel vorbehalten).
            const cfg = (type === 'n' ? NOTES : DISPLAYS)[key] || {};
            setLabel(cfg.label || key);
            const none = document.createElement('span'); none.className = 'gs-op-none'; none.textContent = '–';
