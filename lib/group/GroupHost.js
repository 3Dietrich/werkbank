...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    function sizeFreeGroup(name) {
        ...
        // Eingeklappt NICHT messen (@dpa ddw.md 20260805 Bugfix, „ähnlich bei Gruppen, nur dass
        // es da die Breite betrifft"): ist der Body gerade display:none (Gruppe kollabiert),
        // liefert JEDES Kind `offsetParent === null` — die Schleife unten fände nichts und
        // würde body.style.width/height PERMANENT auf 0px einfrieren (via Inline-Style), auch
        // nach dem Wiederaufklappen (freeGroups-Gruppen sind absolut-positioniert, ihre Kinder
        // tragen nie zur Auto-Breite bei — das eingefrorene 0px bliebe für immer stehen). Neu
        // gemessen wird beim tatsächlichen Aufklappen (applyGroupStyles ruft hier erneut rein).
        if (e.body.style.display === 'none') return;
        let maxR = 0, maxB = 0;
    ...
    function applyGroupStyles(data) {
        ...
        for (const [name, e] of groupEls) {
            ...
            const wasCollapsed = e.body.style.display === 'none';
            e.body.style.display = col ? 'none' : '';
            e.collapseBtn.classList.toggle('collapsed', col);
            // Übergang eingeklappt→sichtbar bei Free-Canvas-Gruppen (@dpa ddw.md 20260805
            // Bugfix, s. sizeFreeGroup()-Kommentar): deren Rahmenmaß wurde beim Einklappen ggf.
            // NICHT gemessen (Guard dort) — jetzt, mit sichtbarem Body, nachholen.
            if (!col && wasCollapsed && freeGroups.has(name)) sizeFreeGroup(name);
        }
        sizePanel();
    }
    let _sizeRaf = null;
    function sizePanel() {
        ...
        _sizeRaf = requestAnimationFrame(() => {
            // Eigene alte Größe VOR der Messung lösen (@dpa ddw.md 20260805 Bugfix, „ähnlich
            // bei Gruppen, nur dass es da die Breite betrifft"): Gruppen sind `position:
            // absolute` mit `width:max-content` (css/main.css) — ihre intrinsische Breite wird
            // für Absolut-Positionierte per Shrink-to-fit INNERHALB der verfügbaren Breite
            // des eigenen containing blocks (= DIESES panel) berechnet. Blieb `panel.style.
            // width/height` von einer FRÜHEREN Messung klein hängen (typischer Auslöser: eine
            // Gruppe war beim Reload eingeklappt), zwang das jede Gruppe beim Wiederaufklappen
            // zurück auf genau diese alte Kleingröße — ein sich selbst bestätigender Zirkel,
            // den auch ein erneuter sizePanel()-Aufruf nie durchbrach (die Gruppe konnte gar
            // nie ihre wahre Breite zeigen, um sie zu messen). Reset vor jeder Messung
            // durchbricht das: die absolut positionierten Kinder legen sich wieder auf ihre
            // echte Breite fest, ERST DANN wird gemessen.
