...
export function mountGroups(root, state, defs, opts = {}) {
    ...
    function makeSelect(key) {
        ...
        registerCtrlStyle('s:' + key, 'select', wrap, (s) => {
            ...
            // „Länge weg, dafür Schriftgröße und Padding").
    ...
    function makeButton(key) {
        ...
        let isOn = false, curBg = '', curBgOn = '', textOn = cfg.label, textOff = cfg.label, outerLabel = '', labelOff = false;
        const defMode = cfg.mode || 'toggle';
        let mode = defMode;   // 'toggle' | 'trigger' | 'gate' (aus ctrlStyles.btnMode, sonst defMode)
        const paint = () => {
            btn.textContent = '';
            const caption = (isOn && textOn) ? textOn : textOff;
        ...
        btn.addEventListener('click', () => { if (mode !== 'gate') activate(); }); // Maus-Gate läuft über mousedown/up
        btn.addEventListener('mousedown', (e) => {
            if (arranging || mode !== 'gate' || e.button !== 0) return;
            e.preventDefault();
        ...
        // getriebenes Ereignis (nächster Takt-Downbeat) den Knopf tatsächlich schaltet.
        ctrlBlinkSetters.set('b:' + key, (on) => { btn.classList.toggle('ctrl-blink', !!on); });
        registerCtrlStyle('b:' + key, 'button', wrap, (s) => {
            outerLabel = s.label || '';
            textOn = s.textOn || cfg.label; textOff = s.textOff || cfg.label;
    ...
    for (const grp of GROUPS) {
        ...
        const segKeys = grp.segments || [], specialCfgs = grp.specials || [];
        let ctrls = null;
        if (selKeys.length || segKeys.length || togKeys.length || btnKeys.length || inlineKeys.length || specialCfgs.length) {
    ...
    // ── Gruppen-Settings (Rechtsklick): Name · BG · Head · Breite · Höhe ────────────
    // Lean-Port: teslacoils Combo-Presets & Gruppen-Snapshots (PickMenu/PresetManager)
    // sind bewusst NICHT dabei — sie hängen an teslacoils Preset-System. Später nachrüstbar.
    let _settingsPop = null, _settingsKey = null;
    ...
    function openGroupSettings(name, anchor) {
        ...
        (() => {
            ...
            num.addEventListener('input', () => {
                ...
                // Größen-Änderungs-Hinweis (@dpa 20260721): NICHT bei manuellem Control-Resize,
                // NUR bei dieser Gruppen-weiten Skalierung — deshalb direkt hier am Eingabefeld
                // ausgelöst, nicht in applyGroupStyles() (das läuft bei JEDER Gruppen-Settings-
                // Änderung, auch Farbe/Breite, würde also falsch-positiv flashen).
                const e = groupEls.get(name);
                if (e) sizeHint.showAll(e.body, 'Größe via Gruppe geändert.');
            });
            r.appendChild(num); sizeGrid.appendChild(r);
        })();
    ...
    return {
        ...
        /** Größen-Änderungs-Hinweis für ALLE Controls (Instrument-weite Skalierung, @dpa 20260721). */
        flashSizeHintAll: (text) => sizeHint.showAll(panel, text),
