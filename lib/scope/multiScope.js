...
// aufwachen, @dpa 20260804 Grill-Runde).
const VALUE_KEYS = ['scopeSrc', 'scopeTriggerPos', 'scopeSyncOffset'];

function scopeName(i) { return i === 0 ? 'Scope' : 'Scope ' + (i + 1); }

/** Sync/Freeze/Trigger-Pos/Sync-Offset als ECHTE GroupHost-Controls (Buttons/Knobs) —
...
function scopeSyncFreezeTpl({ onSyncClick, onFreezeClick }) {
    return {
        BUTTONS: {
            scopeSync: {
...
export function createScopeManager({ host, state, defs, routing, onClockBeat }) {
    ...
    const tpl = scopeSyncFreezeTpl({ onSyncClick: dispatchSync, onFreezeClick: dispatchFreeze });

    // Trigger-Pos/Sync-Offset können sowohl vom Knob (state.set über GroupHost) als auch
    // vom Canvas-Drag (scope.onTriggerPosCommit → state.set, s. buildScope) geändert werden
    // — EIN zentraler Subscribe hält den Scope in beiden Richtungen synchron.
    state.subscribe((k, d) => {
        let m = /^scopeTriggerPos_(\d+)$/.exec(k);
    ...
    function buildScope(i) {
        ...
        host.addGroup({
            name: scopeName(i),
            buttons: ['scopeSync' + sfx, 'scopeFreeze' + sfx],
    ...
    function teardownLast() {
        ...
        delete defs.BUTTONS['scopeSync' + sfx]; delete defs.BUTTONS['scopeFreeze' + sfx];
