...
import { knobsToPorts, buttonsToPorts } from '../routing/portGen.js';
...
export function taktMetroDefs(opts = {}) {
    ...
    const defs = {
        ...
        DEFAULTS: {
            ...
            // teslacoil-Parität (@dpa ddw.md 20260724_212747): das Metronom kann seinen Cutoff
            // an die Base-Frq koppeln — der Klick „stimmt" dann auf die Basis. metroCutoffQuant
            // AN → Cutoff = foldToBand(BaseFrq, metroCutBand) statt des Cutoff-Knobs (engine.js).
            // Die BaseFrq kommt über den Routing-Input `baseFreqIn` (polysynth.baseFreq, s.
            // werkbank.js) — der erste echte VALUE-Modulationsweg „Base-Frq als Quelle".
            metroCutoffQuant: false,
            metroCutBand: 2000,   // untere Bandgrenze L (Hz): BaseFrq wird in [L, 2L) gefaltet
            keyBindings: {
        ...
        KNOBS: {
            ...
            bpm:         { label: 'BPM',             min: 20,  max: 900,   step: 1,   unit: ' BPM', decimals: 0, meta: { viewSize: 'none', dragRange: 60 } },
            // info = Hilfstext fürs Tab-Sonderfenster (@dpa 20260720, selbst formuliert aus
            // taktgeber-Verhalten). Steht in der Tabelle in der Spalte „Hilfstext".
            maxBpm:      { label: 'max. BPM',        min: 100, max: 900,   step: 10,  unit: ' BPM', decimals: 0, meta: { viewSize: 'none' },
                           info: 'Obergrenze fürs Antippen. Wird schneller getippt, halbiert sich der Wert so oft, bis er wieder ins Tempofenster passt.' },
            nudgeAmount: { label: 'Anschieben ±',    min: 1,   max: 20,    step: 1,   unit: ' BPM', decimals: 0, meta: { viewSize: 'none' },
                           info: '±max: die Tiefe, um die + und − das Tempo biegen, solange sie gedrückt sind (der gespeicherte Wert bleibt).' },
            nudgeRampMs: { label: 'Anlauf',          min: 0,   max: 1500,  step: 50,  unit: ' ms',  decimals: 0, meta: { viewSize: 'none' },
                           info: 'Zeit, bis + / − auf ±max kommen — und beim Loslassen genauso wieder zurück.' },
            tapWait:     { label: 'wait (Pause ab)', min: 500, max: 5000,  step: 100, unit: ' ms',  decimals: 0, meta: { viewSize: 'none' },
                           info: 'So lange denkt der Timingmesser nach dem letzten Antippen noch mit. Größere Pause = neue Tap-Serie.' },
            tapTol:      { label: '±Fenster',        min: 5,   max: 40,    step: 1,   unit: ' %',   decimals: 0, meta: { viewSize: 'none' },
                           info: 'Toleranz um das laufende Tempo: liegt ein Antipper darin, zählt seine Abweichung mit, sonst gilt er als Ausreißer.' },
            tapWin:      { label: 'Fenster (M2)',    min: 2,   max: 12,    step: 1,   decimals: 0,  meta: { viewSize: 'none' },
                           info: 'Nur im Modus „folgend": gleitendes Mittelungs-Fenster in Schlägen. Klein = folgt Tempo-Drifts schnell, groß = ruhiger.' },
            ...
            latencyOffset: { label: 'Latenz', min: -1000, max: 1000, step: 10, unit: ' ms', decimals: 0, meta: { viewSize: 'none' },
                           info: 'Schiebt Klick + Anzeige gegen einen externen Klick: − = früher (schluckt die Ausgabelatenz), + = später.' },
            ...
            // Band-Regler für die Base-Frq-Kopplung (nur wirksam bei metroCutoffQuant): der Cutoff
            // ist die in [L, 2L) gefaltete BaseFrq. Anzeige „L–2L" wie beim Poly-Synth-baseBand.
            metroCutBand:      { label: 'Band',       min: 20,  max: 9000,  step: 10,   unit: ' Hz', decimals: 0, curve: 'log',
                                 formatValue: (v) => `${Math.round(v)}–${Math.round(v) * 2}`,
                                 info: 'Nur bei „Cut⇢Base": der Metro-Cutoff ist die in [L, 2L) gefaltete Base-Frq — L ist diese untere Bandgrenze.' },
        ...
        SEGMENTS: {
            tapMode: { label: 'Modus',
                info: '„Konstant" mittelt über alle Antipper und rastet auf ein stabiles Tempo ein. „Folgend" nimmt nur die jüngsten Schläge (Fenster) und synct sich live mit jedem gültigen Schlag — folgt so Tempo-Drifts.',
                options: [
        ...
        TOGGLES: {
            ...
            // teslacoil-Parität: AN → der Metro-Cutoff folgt der Base-Frq (in [Band, 2·Band]
            // gefaltet), der Cutoff-Knob ist dann wirkungslos. Der Klick „stimmt" auf die Basis.
            metroCutoffQuant: { label: 'Cut⇢Base', title: 'Metro-Cutoff folgt der Base-Frq (in [Band, 2·Band] gefaltet) statt dem Cutoff-Knob.' },
        },
        ...
        BUTTONS: {
            // Zwei getrennte Starts (@dpa ddw.md 20260724_212747): früher triggerte EIN Start
            // implizit den Sync mit („alles von vorne"). Jetzt aufgeteilt — beide sind Toggle
            // (nochmal drücken stoppt, Stop-Verhalten unverändert) und tragen je einen eigenen
            // Key/MIDI-Learn (b:start / b:startCont). '>' = mit avv (Takt→1, Seq→Step 0),
            // '|>' = weiter ohne avv (Phase/Position laufen weiter).
            start:     btn('>',  'start',     'Start von vorne: Takt auf die 1, Sequenzer auf Step 0 (avv). Nochmal drücken = Stop.', 'toggle'),
            startCont: btn('|>', 'startCont', 'Weiter ohne Sync: Takt-Phase und Sequenzer-Position laufen von dort weiter, wo sie standen. Nochmal drücken = Stop.', 'toggle'),
        ...
        GROUPS: [
            {
                ...
                buttons: ['start', 'startCont', 'bang', 'bang2', 'slow', 'fast', 'tap', 'tapReset'],
                knobs: ['bpm'],
                ...
                specials: [{
                    ...
                    // layout:'table' → Spalten Label | Einstellung | Hilfstext (@dpa 20260720).
                    layout: 'table',
                    desc: 'Tap-Tempo & Timing: oben mit „Tab" mittippen, hier feineinstellen. Der Modus legt fest, wie aus den Antippern ein Tempo wird — der Rest formt das Timing.',
                    // Echte Ausgabelatenz + Samplerate der Audio-Engine (opts.audioInfo, aus
                    // werkbank.js verdrahtet). GENAU, nicht geschätzt.
                    audioInfo: opts.audioInfo || null,
                    segments: ['tapMode'],
            ...
            {
                name: 'Takt / Metronom',
                toggles: ['metroOn', 'metroCutoffQuant'],
                displays: ['beatView'],
                knobs: ['beatsPerBar', 'metroLevel', 'metroMorph', 'metroCutoff', 'metroCutBand', 'metroCutoffRatio', 'metroReso'],
            },
        ],
        ...
        // inputs unten NACH dem Objektbau ergänzt (Punkt 3b) — braucht KNOBS/BUTTONS, die hier
        // noch nicht als Variable existieren (waren nie extrahiert, s. defs.KNOBS/defs.BUTTONS).
        ports: {
            outputs: [
                { id: 'beat', label: 'Clock-Beat', type: 'Gate' },
            ],
            inputs: [],
        },
    };

    // Punkt 3b (ddw.md 20260724): ALLE Knobs + ALLE Buttons dezentral aus KNOBS/BUTTONS
    // abgeleitet (portGen.js) — dieselbe Quelle wie das Panel, keine doppelte Pflege.
    // Zusätzlich der Modulations-Input `baseFreqIn` (ddw.md 20260724_212747, „Base-Frq als
    // Quelle"): ein OSZ-F-VALUE-Port, den werkbank.js dauerhaft mit polysynth.baseFreq
    // verbindet. Wirksam nur bei metroCutoffQuant — die Zustellung (flush) ist billig/idempotent.
    defs.ports.inputs = [
        ...knobsToPorts(defs.KNOBS), ...buttonsToPorts(defs.BUTTONS),
        { id: 'baseFreqIn', label: 'Base-Frq ⇢ Cutoff', type: 'OSZ-F' },
    ];
    return defs;
}
