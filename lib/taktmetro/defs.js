...
export function taktMetroDefs(opts = {}) {
    ...
    const btn = (label, act, title) => ({ label, title, onClick: () => onAction(act) });
    ...
    return {
        ...
        KNOBS: {
            ...
            metroCutoffOffset: { label: 'Cutoff Teil 2', min: -8000, max: 8000, step: 50, unit: ' Hz', decimals: 0 },
        ...
        BUTTONS: {
            start:    btn('▶ / ⏸', 'start', 'Start/Stop.'),
            bang:     btn('!',  'bang',  'Der nächste Schlag wird zur 1.'),
            bang2:    btn('!!', 'bang2', 'Die 1 fällt sofort.'),
            slow:     btn('−',  'slow',  'Bremsen, solange gedrückt.'),
            fast:     btn('+',  'fast',  'Anschieben, solange gedrückt.'),
            tap:      btn('Tab', 'tap',  'Mittippen.'),
            tapReset: btn('↺', 'tapReset', 'Tap-Serie verwerfen (laufendes Tempo bleibt).'),
        },
        ...
        GROUPS: [
            ...
            },
            {
                ...
                toggles: ['metroOn'],
                displays: ['beatView'],
                knobs: ['beatsPerBar', 'metroLevel', 'metroMorph', 'metroCutoff', 'metroCutoffOffset', 'metroReso'],
            },
        ],
    };
}
