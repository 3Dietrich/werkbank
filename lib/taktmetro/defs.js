...
export function taktMetroDefs(opts = {}) {
    ...
    const defs = {
        ...
        DEFAULTS: {
            ...
            // Base-Frq-Kopplung (@dpa ddw.md 20260724_212747, korrigiert 233253): das Metronom
            // kann seinen Cutoff an die Base-Frq koppeln — der Klick „stimmt" dann auf die Basis.
            // metroCutoffQuant (Quant) AN → beide Metro-Cutoffs rasten auf das nächste Vielfache
            // der Base-Frq (engine.js), „rundet die Cutoffs". Der Band-Regler ist wieder raus
            // (@dpa: „Band erstmal raus, vielleicht aber auch wieder"). Die BaseFrq kommt über den
            // Routing-Input `baseFreqIn` (polysynth.baseFreq) — der erste VALUE-Modulationsweg.
        ...
        TOGGLES: {
            ...
            // AN → die Metro-Cutoffs werden auf das nächste Vielfache der Base-Frq gerundet, der
            // Klick „stimmt" harmonisch auf die Basis (@dpa 20260724_233253: „Quant … rundet die Cutoffs").
            metroCutoffQuant: { label: 'Quant', title: 'Rundet die Metro-Cutoffs auf das nächste Vielfache der Base-Frq (Klick „stimmt" auf die Basis).' },
        },
        ...
        BUTTONS: {
            // Zwei getrennte Starts, beide Toggle (nochmal drücken stoppt, Stop-Verhalten
            // unverändert), je eigener Key/MIDI-Learn (b:start / b:startCont). Zuordnung nach
            // @dpa-Korrektur (ddw.md 20260724_233253): '>' läuft AB POSITION weiter (continue),
            // '|>' startet VON VORNE (avv: Takt→1, Seq→Step 0). Die Semantik (keepPhase) sitzt
            // in engine.js onAction — hier nur Label + Hilfstext dazu.
            start:     btn('>',  'start',     'Weiter ohne Sync: Takt-Phase und Sequenzer-Position laufen von dort weiter, wo sie standen. Nochmal drücken = Stop.', 'toggle'),
            startCont: btn('|>', 'startCont', 'Start von vorne: Takt auf die 1, Sequenzer auf Step 0 (avv). Nochmal drücken = Stop.', 'toggle'),
    ...
    defs.ports.inputs = [
        // Gruppen-Namen für „ISM / Gruppe → Control" (@dpa 20260725) aus GROUPS abgeleitet.
        ...knobsToPorts(defs.KNOBS, undefined, groupOfControl(defs.GROUPS)),
        ...buttonsToPorts(defs.BUTTONS, undefined, groupOfControl(defs.GROUPS)),
