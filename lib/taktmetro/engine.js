 ...
 *  • Teil 2 = Teil 1 + `metroCutoffOffset` (aus DEFAULTS 4000, jetzt als Regler).
 *  • slow/fast: EINMALIGER Nudge um ±`nudgeAmount` (taktgebers Halten-zum-Biegen braucht
 *    mousedown/up am Button — das ist eine GroupHost-Erweiterung, hier bewusst noch nicht;
 *    fürs Hören des Klicks reicht der einmalige Schub).
 */
...
export function createTaktEngine(state) {
    ...
    function rebuildMetro() {
        ...
        const cut2 = Math.max(50, Math.min(18000, cut + (state.get('metroCutoffOffset') || 0)));
    ...
    function ensureAudio() {
        ...
        clock.bpmFn = () => Math.max(1, state.get('bpm'));
    ...
    function nudge(dir) {
        const amt = state.get('nudgeAmount') || 2;
        const max = state.get('maxBpm') || 900;
        state.set('bpm', Math.max(1, Math.min(max, state.get('bpm') + dir * amt)));
    }
    ...
    state.subscribe((k, d) => {
        if (k === 'metroMorph' || k === 'metroCutoff' || k === 'metroReso' || k === 'metroCutoffOffset') rebuildMetro();
    ...
    function onAction(id) {
        switch (id) {
            ...
            case 'slow': nudge(-1); break;
            case 'fast': nudge(1); break;
            case 'tap': tap(); break;
