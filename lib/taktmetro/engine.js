...
import { foldToBand } from '../polysynth/pitch/Scaler.js';
...
export function createTaktEngine(state) {
    ...
    // metroCutoffQuant AN ist — dann ist der Metro-Cutoff die daran gefaltete Base-Frq.
    let _baseFreqIn = 0;
    ...
    function rebuildMetro() {
        if (!metro) return;
        // Cutoff normal aus dem Knob; bei metroCutoffQuant stattdessen die Base-Frq, in
        // [Band, 2·Band) gefaltet (teslacoil-Parität, foldToBand aus Scaler.js). foldToBand
        // gibt bei baseFreq<=0 den Rohwert zurück → Fallback bleibt der Cutoff-Knob.
        let cut = state.get('metroCutoff');
        if (state.get('metroCutoffQuant') && _baseFreqIn > 0) {
            cut = Math.max(50, Math.min(18000, foldToBand(_baseFreqIn, state.get('metroCutBand'))));
        }
    ...
    state.subscribe((k, d) => {
        if (k === 'metroMorph' || k === 'metroCutoff' || k === 'metroReso' || k === 'metroCutoffRatio'
            || k === 'metroCutoffQuant' || k === 'metroCutBand') rebuildMetro();
