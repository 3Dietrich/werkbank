...
export function createTaktEngine(state) {
    ...
    // Roher Scheduler-Beat (time, beatInBar) — Listener-LISTE statt Einzel-Callback (@dpa
    // 20260804, Signal-Scope-Sync-Grill-Runde): bisher nur EIN Abonnent (Rec/Sequenzer,
    // per EINEM `taktEngine.onClockBeat(fn)`-Aufruf je Einstiegsdatei) — ein zweiter Aufruf
    // hätte den ersten stillschweigend überschrieben. Jetzt mehrere gleichzeitige Abonnenten
    // möglich (z.B. mehrere Signal-Scope-Instanzen im Sync-Modus), ohne sich gegenseitig zu
    // verdrängen. `onClockBeat(fn)` gibt eine Unsubscribe-Funktion zurück; bestehende
    // Einmal-Aufrufer (Rec/Seq) ignorieren den Rückgabewert einfach weiter — reines Add,
    // kein Verhaltensunterschied für sie.
    const _clockBeatListeners = new Set();
    ...
    function ensureAudio() {
        ...
        clock = new Clock(audio, (t, beat) => {
            metroTick(t, beat); scheduleBeat(t, beat);
            for (const fn of _clockBeatListeners) fn(t, beat);
        });
    ...
    return {
        ...
        // (time, beatInBar) roh, MEHRERE Abonnenten möglich (Rec/Seq + Signal-Scope-Sync).
        // Gibt eine Unsubscribe-Funktion zurück (von Abonnenten, die sich wieder abmelden
        // müssen, z.B. Scope-Instanzen bei Sync-Aus/Löschen — Rec/Seq ignorieren sie).
        onClockBeat(fn) {
            if (!fn) return () => {};
            _clockBeatListeners.add(fn);
            return () => _clockBeatListeners.delete(fn);
        },
        offClockBeat(fn) { _clockBeatListeners.delete(fn); },
