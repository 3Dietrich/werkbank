...
export function createTaktEngine(state) {
    ...
    let recArmed = null;   // null | 'start' | 'stop' — Rec-Instrument-TODO 5 (Start/Stop-Sync)
    ...
    let _onRecording = () => {};   // Callback bei tatsächlichem Rec-Start/Stop (Knopf ON-Farbe)
    let _onRecArmed = () => {};    // Callback bei Arm/Entarm (Knopf-Blink, „wartet auf 0C")
    ...
    function ensureAudio() {
        ...
        clock = new Clock(audio, (t, beat) => { metroTick(t, beat); scheduleBeat(t, beat); checkRecArm(t, beat); });
    ...
    function recStart() {
        ...
        recorder.onstop = () => {
            ...
            processRecording(rawBlob, {
                audioContext: audio, bpm: state.get('bpm') || 120,
                beatsPerBar: Math.max(1, state.get('beatsPerBar') | 0) || 4,
    ...
    // Start/Stop-Sync (@dpa 20260721, Rec-Instrument-TODO 5): Rec schaltet NICHT sofort, sondern
    // „armed" sich (sichtbar blinkend) und wartet auf den nächsten Takt-Downbeat (beatInBar===0)
    // — auch wenn der Takt gerade noch gar nicht läuft (dann wartet es, bis er startet). So
    // lassen sich nahtlose Loops aufnehmen. Ein Klick während eines laufenden Arms tut nichts —
    // erst wenn der Arm sich auflöst (0C erreicht oder recArmed()===null), reagiert der nächste
    // Klick wieder normal. checkRecArm() (im Clock-Callback) löst den Arm sample-genau ein,
    // nach demselben Muster wie scheduleBeat() (setTimeout auf den geplanten AudioContext-Zeitpunkt).
    function recToggle() {
        if (recArmed) return;
        recArmed = recording() ? 'stop' : 'start';
        _onRecArmed(recArmed);
        // GroupHost flippt den Knopf beim Klick optimistisch (mode:'toggle', „richtet sich
        // danach gerade" — normal same-frame korrekt, s. Start-Knopf). Weil Rec jetzt asynchron
        // erst am nächsten 0C schaltet, muss hier synchron der TATSÄCHLICHE Stand re-asserted
        // werden, sonst zeigt der Knopf sofort fälschlich „läuft/steht", bevor es wirklich so ist.
        _onRecording(recording());
    }
    function checkRecArm(time, beatInBar) {
