    ...
    const build = (list, binds) => {
        ...
        for (const p of list || []) {
            ...
            out[p.id] = {
                ...
                group: p.group,   // Gruppenname für „ISM / Gruppe → Control" (@dpa 20260725)
                // hasNode/node (ddw.md 20260727, Scope „sample"-Genauigkeit): optional, NUR
                // wenn der Output tatsächlich einen echten AudioNode im Graph hat (z.B. ADSR-
                // EnvEngine → ConstantSourceNode). hasNode ist ein billiger, seiteneffektfreier
                // Fähigkeits-Check für UI-Greying; node() ist der LAZY Getter, der den Node bei
                // Bedarf erst erzeugt (nie beim bloßen Prüfen/Öffnen von Settings triggern).
                hasNode: b.hasNode, node: b.node,
            };
        }
        return out;
    };
...
// Default-Key trägt das Präfix DIESES Einstiegs (@dpa dd.md 20260801_2, s. lib/appId.js):
// Verdrahtungen zeigen auf Modul-IDs, die es nur auf ihrer eigenen Seite gibt (index.html
// hat polysynth/stepseq, werkbank-leer.html nicht). Über einen geteilten Key hätte jede
// Seite die Verbindungen der anderen geladen und die unbekannten still verworfen — beim
// nächsten Speichern wären sie damit auch dort weg gewesen.
export function createRoutingRegistry({ stateKey = lsKey('routing') } = {}) {
    ...
    function inputTargets(srcType) {
        ...
        for (const [mid, m] of modules) {
            for (const [pid, p] of Object.entries(m.inputs)) {
                ...
                out.push({
                    // „ISM / Gruppe → Control" (@dpa 20260725: bisher nur „ISM → Control" —
                    // ohne Gruppen-Herleitung war z.B. nicht erkennbar, WO am ISM das Ziel
                    // sitzt). Ports ohne Gruppen-Angabe (Sonder-Ports wie baseFreqIn) fallen
                    // auf das alte Format zurück.
                    name: `${t(m.label)}${p.group ? ' / ' + t(p.group) : ''} → ${t(p.label)}`, module: mid, port: pid,
    ...
    /** Alle Output-Ports IM ENSEMBLE, die einen lesbaren Wert liefern (`read` gesetzt) —
     *  Pendant zu `inputTargets()`, aber für MITLESEN statt Zustellen (@dpa 20260726:
     *  „ein Meter ist eine ANZEIGE, die braucht kein Output, sondern die Quelle, die sie
     *  anzeigt — OHNE den Fluss zum eigentlichen Ziel zu unterbrechen"). Ein Meter/Scope
     *  ruft NIE `write()` auf einem fremden Port — er hängt sich nur mit `getValue()`
     *  LESEND an eine Quelle, der bestehende Signalweg bleibt komplett unberührt. */
    function outputSources() {
        const out = [];
        for (const [mid, m] of modules) {
            for (const [pid, p] of Object.entries(m.outputs)) {
                if (!p.read) continue;   // Event-Ports (kein read) sind keine Meter-Quelle
                out.push({
                    name: `${t(m.label)}${p.group ? ' / ' + t(p.group) : ''} → ${t(p.label)}`,
                    module: mid, port: pid, type: p.type,
                    // s. bindPorts()-Kommentar: reine Durchreichung fürs Scope-„sample"-Feature.
                    hasNode: p.hasNode, node: p.node,
                });
            }
        }
        return out;
    ...
    return {
        ...
        emit, deliver, inputTargets, outputSources, getValue, flush,
