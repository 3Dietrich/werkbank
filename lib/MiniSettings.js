...
export class MiniSettings {
   ...
   * @param {{get:()=>({x:number,y:number}|undefined), set:(pos:{x:number,y:number})=>void}} [posStore]
   *        Position merken (ddw.md 20260726) — dieses Panel kennt selbst keinen State (s.
   *        Kopfkommentar), darum liefert der Aufrufer get/set (meist ein einzelner State-Key).
   */
  open(anchor, build, onClose, posStore) {
    this.close();
    this._onClose = onClose || null;
    this._posStore = posStore || null;
    ...
    const savedPos = this._posStore && this._posStore.get();
    if (savedPos) {
    ...
    makeDraggable(panel, panel.querySelector('.kme-header'), (pos) => {
      if (this._posStore) this._posStore.set(pos);
    });
